...
(function( window ) {
    ...
    function define_library(){
        ...
        Form.initialize = function(){
            ...
            while(text = re.exec(document.body.innerHTML)) {
                ...
                if (Core.debug()) console.log(replaceArgs.data['style']+' MailVotech Form: '+replaceText);
                        ...
                        src: async (query) => {
                            try {
                                const source = await fetch(
                                    ...
                                    MailVotechDomain + '/form/company-lookup/autocomplete',
                                    {
                                        method: 'POST',
                                        body: JSON.stringify({
                                            search: query,
                                            formId: autoCompleteJS.input.closest('form').querySelector('input[name="mailvotechform[formId]"]').value
                                        })
                                    }
                                );
        ...
        Form.bindClickEvents = function() {
            ...
            for(var index in Form.clickEvents) {
                ...
                document.querySelector(current.data.element).setAttribute("_mailvotech_form_index", index);
                document.querySelector(current.data.element).addEventListener("click", function(){
                    ...
                    Form.openModal(Form.clickEvents[this.getAttribute('_mailvotech_form_index')]);
                });
            }
        };
        ...
        Form.getFormLink = function(options) {
            return Core.getMailVotechBaseUrl() + 'index.php/form/' + options.data['id'] + '?' + options.params;
        };
        ...
        Form.customCallbackHandler = function(formId, event, data) {
            if (typeof MailVotechFormCallback !== 'undefined' &&
                typeof MailVotechFormCallback[formId] !== 'undefined' &&
                typeof MailVotechFormCallback[formId][event] == 'function'
            ) {
                if (typeof data == 'undefined') {
                    data = null;
                }

                return MailVotechFormCallback[formId][event](data);
            }
        ...
        Form.prepareForms = function() {
            ...
            for (var i = 0, n = forms.length; i < n; i++) {
                var formId = forms[i].getAttribute('data-mailvotech-form');
                if (formId !== null) {
                    ...
                    const hasPageBreak = theForm.querySelectorAll('[data-mailvotech-form-page]').length > 1;
                    if (hasPageBreak) {
        ...
        Form.syncSliderOutputs = function(formId) {
            const theForm = document.getElementById('mailvotechform_' + formId);
            if (!theForm) return;
            const outputs = theForm.querySelectorAll('output.mailvotechform-slider-value');
            [].forEach.call(outputs, function(out) { out.textContent = ''; });
        };

        Form.prepareMessengerForm = function(formId) {
            var theForm = document.getElementById('mailvotechform_' + formId);
            ...
            if (!document.getElementById('mailvotechiframe_' + formId)) {
                ...
                ifrm.style.width = 0;
                ifrm.style.heigh = 0;
                ifrm.setAttribute( 'id', 'mailvotechiframe_' + formId);
                ifrm.setAttribute('name', 'mailvotechiframe_' + formId);
                document.body.appendChild(ifrm);

                theForm.target = 'mailvotechiframe_' + formId;
            }

            if (!document.getElementById('mailvotechform_' + formId + '_messenger')) {
                var messengerInput = document.createElement('INPUT');
                messengerInput.type = 'hidden';
                messengerInput.setAttribute('name', 'mailvotechform[messenger]');
                messengerInput.setAttribute('id', 'mailvotechform_' + formId + '_messenger');
        ...
        Form.prepareValidation = function(formId) {
            if (typeof window.MailVotechFormValidations[formId] == 'undefined') {
                window.MailVotechFormValidations[formId] = {};
                ...
                [].forEach.call(validations, function (container) {
                    ...
                    window.MailVotechFormValidations[formId][alias] = {
        ...
        Form.prepareShowOn = function (formId) {
            var theForm = document.getElementById('mailvotechform_' + formId);
            var showOnDataAttribute = 'data-mailvotech-form-show-on';
            ...
            [].forEach.call(showOn, function (container) {
                ...
                var idOnChangeElem  = "mailvotechform_" + formId+"_"+returnArray[0];
        ...
        Form.doShowOn = function (parents, key, selectedValues) {
            ...
            Object.keys((parents[key])).forEach(function(key2) {
                [].forEach.call(selectedValues, function (selectedValue) {
                    ...
                    if (selectedValue) {
                        if (el.getAttribute('data-mailvotech-form-expr') == 'notIn') {
        ...
        Form.preparePagination = function(formId) {
            var theForm        = document.getElementById('mailvotechform_'+formId);
            var pages          = theForm.querySelectorAll('[data-mailvotech-form-page]');
            var lastPageNumber = pages.length;

            [].forEach.call(pages, function (page) {
                var pageNumber       = parseInt(page.getAttribute('data-mailvotech-form-page'));
                var pageBreak        = theForm.querySelector('[data-mailvotech-form-pagebreak="'+pageNumber+'"]');

                if (pageNumber > 1) {
                    // Hide other pages by default
                    page.classList.add('mailvotechform-page-hidden');
                }
                ...
                if (pageBreak) {
                    ...
                    var prevButton = pageBreak.querySelector('[data-mailvotech-form-pagebreak-button="prev"]');
                    var nextButton = pageBreak.querySelector('[data-mailvotech-form-pagebreak-button="next"]');
                    ...
                    nextButton.onclick = function(formId, theForm, hidePageNumber, showPageNumber) {
                        return function () {
                            ...
                            var validations = theForm.querySelector('[data-mailvotech-form-page="' + hidePageNumber + '"]').querySelectorAll('[data-validate]');
                            var isValid = true;
                            [].forEach.call(validations, function (container) {
                    ...
                    if (1 === pageNumber) {
                        ...
                        pageBreak.classList.remove('mailvotechform-page-hidden');
                    } else {
        ...
        Form.switchPage = function(theForm, showPageNumber) {
            ...
            [].forEach.call(pages, function (page) {
                ...
                var pageBreak  = theForm.querySelector('[data-mailvotech-form-pagebreak="'+pageNumber+'"]');
                if (pageBreak) {
                    pageBreak.classList.add('mailvotechform-page-hidden');
                }
            });

            // Show the wanted page
            var thePage = theForm.querySelector('[data-mailvotech-form-page="' + showPageNumber + '"]');
            if (thePage) {
                thePage.classList.remove('mailvotechform-page-hidden');
            }
            var showPageBreak = theForm.querySelector('[data-mailvotech-form-pagebreak="' + showPageNumber + '"]');
            if (showPageBreak) {
                showPageBreak.classList.remove('mailvotechform-page-hidden');
            }
        };
        ...
        Form.getPageForField = function (formId, fieldId, switchPage) {
            ...
            var pageBreak = Form.findAncestor(container, 'mailvotechform-page-wrapper');
            if (pageBreak) {
                var page = pageBreak.getAttribute('data-mailvotech-form-page');
                if (switchPage) {
                    Form.switchPage(document.getElementById('mailvotechform_' + formId), page);
                }
        ...
        Form.findAncestor = function (el, cls) {
            ...
            while (true) {
                ...
                if (!parent || Form.hasClass(parent, 'mailvotechform-innerform')) {
                    break;
        ...
        Form.validator = function(formId) {
            var validator = {
                validateForm: function (submitForm) {
                    ...
                    var elId              = 'mailvotechform_' + formId;
                    ...
                    if (formValid === null) {
                        ...
                        var formContainer = document.getElementById('mailvotechform_wrapper_' + formId);
                        if (formContainer) {
                            formContainer.className = formContainer.className.replace(" mailvotechform-post-success", "");
                        }
                        ...
                        for (var fieldKey in MailVotechFormValidations[formId]) {
                    ...
                    if (formValid && submitForm) {
                        ...
                        if (MailVotechFormValidations[formId] && MailVotechFormValidations[formId][firstInvalidField]) {
                            const invalidField = MailVotechFormValidations[formId][firstInvalidField];
                            const invalidElement = document.getElementsByName('mailvotechform[' + invalidField.name + ']')[0];
                            if (invalidElement) {
                ...
                validateField: function(theForm, fieldKey) {
                    var field = MailVotechFormValidations[formId][fieldKey];
                    ...
                    if (valid === null) {
                        var name = 'mailvotechform[' + field.name + ']';
                ...
                markError: function(containerId, valid, validationMessage) {
                    ...
                    if (elContainer) {
                        elErrorSpan = elContainer.querySelector('.mailvotechform-errormsg');
                    ...
                    if (!Form.customCallbackHandler(formId, 'onErrorMark', callbackData)) {
                        if (elErrorSpan) {
                            ...
                            elContainer.className = elContainer.className + " mailvotechform-has-error";
                        }
                    }
                },
                ...
                clearErrors: function() {
                    var theForm    = document.getElementById('mailvotechform_' + formId);
                    var hasErrors  = theForm.querySelectorAll('.mailvotechform-has-error');
                    var that       = this;
                ...
                clearError: function(containerId) {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onErrorClear', containerId)) {
                        var elContainer = document.getElementById(containerId);
                        if (elContainer) {
                            var elErrorSpan = elContainer.querySelector('.mailvotechform-errormsg');
                            if (elErrorSpan) {
                                elErrorSpan.style.display = 'none';
                                elErrorSpan.setAttribute('aria-hidden', 'true');
                                elContainer.className = elContainer.className.replace(" mailvotechform-has-error", "");
                            }
                        }
                    }
                },
                ...
                parseFormResponse: function (response) {
                    ...
                    var ifrm = document.getElementById('mailvotechiframe_' + formId);
                    if (ifrm) {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onResponse', response)) {
                        ...
                        if (response.download) {
                            ...
                            document.getElementById('mailvotechiframe_' + formId).src = response.download;
                            ...
                            for (var field in response.validationErrors) {
                                ...
                                this.markError('mailvotechform_' + formId + '_' + field, false, response.validationErrors[field]);
                            }

                            if (firstPage) {
                                Form.switchPage(document.getElementById('mailvotechform_' + formId), firstPage);
                            }
                        ...
                        if (response.success) {
                            if (response.hideform) {
                                this.fadeOut(document.getElementsByClassName('mailvotechform-innerform')[0]);
                            }
                            ...
                            var formContainer = document.getElementById('mailvotechform_wrapper_' + formId);
                            if (formContainer) {
                                formContainer.className = formContainer.className + " mailvotechform-post-success";
                            }
                ...
                setMessage: function (message, type) {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onMessageSet', {message: message, type: type})) {
                        var container = document.getElementById('mailvotechform_' + formId + '_' + type);
                        if (container) {
                ...
                resetForm: function () {
                    ...
                    Form.switchPage(document.getElementById('mailvotechform_' + formId), 1);

                    document.getElementById('mailvotechform_' + formId).reset();
                ...
                disableSubmitButton: function() {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onSubmitButtonDisable')) {
                        var submitButton = document.getElementById('mailvotechform_' + formId).querySelector('.mailvotechform-button');

                        if (submitButton) {
                            MailVotechLang.submitMessage = submitButton.innerHTML;
                            submitButton.innerHTML = MailVotechLang.submittingMessage;
                ...
                enableSubmitButton: function() {
                    ...
                    if (!Form.customCallbackHandler(formId, 'onSubmitButtonEnable')) {
                        var submitButton = document.getElementById('mailvotechform_' + formId).querySelector('.mailvotechform-button');
                        if (submitButton) {
                            submitButton.innerHTML = MailVotechLang.submitMessage;
        ...
        Form.registerFormMessenger = function() {
            window.addEventListener('message', function(event) {
                ...
                if (MailVotechDomain.indexOf(event.origin) !== 0) return;
        ...
        Form.getFieldContainerId = function(formId, fieldKey) {
            var containerId = 'mailvotechform_' + formId + '_' + fieldKey;
            if (!document.getElementById(containerId)) {
                containerId = 'mailvotechform_' + fieldKey;
            }
        ...
        Form.populateValuesWithGetParameters = function() {
            if (document.forms.length !== 0 && window.location.search) {
                ...
                for (const entry of entries) {
                    const inputs = document.getElementsByName(`mailvotechform[${entry[0]}]`);
        ...
        Modal.loadStyle = function() {
            ...
            s.rel = "stylesheet"
            s.type = "text/css"
            s.href = Core.debug() ? Core.getMailVotechBaseUrl() + 'app/assets/css/modal.css' : Core.getMailVotechBaseUrl() + 'media/css/modal.min.css';
        ...
        Modal.open = function() {
            ...
            this.modal.className = this.modal.className + (this.modal.offsetHeight > window.innerHeight ? " mailvotechForm-open mailvotechForm-anchored" : " mailvotechForm-open");
            this.overlay.className = this.overlay.className + " mailvotechForm-open";
        };
        ...
        Modal.buildOut = function() {
            ...
            this.modal.className = "mailvotechForm-modal " + this.options.className;
            ...
            if (this.options.closeButton === true) {
                ...
                this.closeButton.className = "mailvotechForm-close close-button";
            ...
            if (this.options.overlay === true) {
                ...
                this.overlay.className = "mailvotechForm-overlay " + this.options.className;
                docFrag.appendChild(this.overlay);
            }

            // Create content area and append to modal
            contentHolder = document.createElement("div");
            contentHolder.className = "mailvotechForm-content";
        ...
        Modal.close = function() {
            var _ = this;
            this.modal.className = this.modal.className.replace(" mailvotechForm-open", "");
            this.overlay.className = this.overlay.className.replace(" mailvotechForm-open", "");
        ...
        Core.setMailVotechBaseUrl = function(base_url) {
            config.mailvotech_base_url = base_url.split('/').slice(0,-3).join('/')+'/';
        };

        // Be aware that this base URL may contain asset_prefix if set.
        Core.getMailVotechBaseUrl = function() {
            return config.mailvotech_base_url;
        };

        Core.initialize = function(base_url) {
            Profiler.startTime();
            if (Core.debug()) console.log('SDK initialized');
            if (typeof(config.mailvotech_base_url) == 'undefined') Core.setMailVotechBaseUrl(base_url);
            if (Core.debug()) console.log('Automatic setup mailvotech_base_url as: ' + config.mailvotech_base_url);
            Modal.loadStyle();
    ...
    if (typeof(MailVotechSDK) === 'undefined') {
        window.MailVotechSDK = define_library();
        var sjs = document.getElementsByTagName('script'), tjs = sjs.length;
        for (var i = 0; i < sjs.length; i++) {
            if (!sjs[i].hasAttribute('src') || sjs[i].getAttribute("src").indexOf('mailvotech-form-src.js') == -1) continue;
            var sParts = sjs[i].getAttribute("src").split("?");
            if (sParts[1]) MailVotechSDK.setConfig(MailVotechSDK.parseToObject(sParts[1]));
            MailVotechSDK.initialize(sParts[0]);
            break;
        }
    }
    if (typeof window.MailVotechFormValidations == 'undefined') {
        window.MailVotechFormValidations = {};
    }
})( window );
