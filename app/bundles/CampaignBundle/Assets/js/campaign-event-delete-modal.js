...
MailVotech.campaignEventDeleteModal = {
        ...
        this.confirmButton.on('click', () => {
            ...
            if (!selectedEventId) {
                ...
                this.warningDiv.find('.warning-message').html(
                    MailVotech.translate('mailvotech.campaign.event.delete.select_redirect')
                );
            ...
            const warningMessage = MailVotech.translate('mailvotech.campaign.event.delete.confirmation');
            ...
            setTimeout(() => {
                ...
                if (typeof MailVotech.campaignBuilderInstance !== 'undefined') {
                    mQuery("#CampaignCanvas .list-campaign-event a[data-toggle='ajax-delete']").each(function() {
                        ...
                        mQuery(this).off('click.ajax').on('click.ajax', MailVotech.handleEventDeleteClick);
                    });
                }
            }, 100);
        });
    ...
    populateEventOptions: function(currentEventId) {
        ...
        const availableEvents = MailVotech.getCampaignBuilderEventOptions(currentEventId);

        if (availableEvents.length === 0) {
            // Show an error message - can't delete the last event
            this.redirectLabel.html(`<span class='text-danger'>${MailVotech.translate('mailvotech.campaign.event.delete.unable_delete_last')}</span>`);
            this.redirectSelect.removeClass('required');
            this.confirmButton.removeClass('btn-danger').addClass('btn-default').text(MailVotech.translate('mailvotech.campaign.event.delete'));
            this.confirmButton.prop('disabled', true);

            // Add error message
            this.warningDiv.removeClass('alert-warning').addClass('alert-danger').show();
            this.warningDiv.find('.warning-message').html(
                MailVotech.translate('mailvotech.campaign.event.delete.last_event')
            );
        ...
        this.redirectLabel.html(`${MailVotech.translate('mailvotech.campaign.event.delete.redirect_contacts')} <span class='text-danger'>*</span>`);
        this.confirmButton.removeClass('btn-default').addClass('btn-danger').text(MailVotech.translate('mailvotech.campaign.event.delete.redirect_event'));
        this.confirmButton.prop('disabled', false);
        ...
        mQuery.each(availableEvents, (index, event) => {
            const optionLabel = '('+ MailVotech.translate('mailvotech.campaign.event.id') +': ' + event.id+ ') (' +
    ...
    hasDelay: function(eventId) {
        if (typeof MailVotech.campaignBuilderCanvasEvents === 'undefined'
            || !MailVotech.campaignBuilderCanvasEvents[eventId]) {
            return false;
        }

        const event = MailVotech.campaignBuilderCanvasEvents[eventId];
    ...
    isRedirectTarget: function(eventId) {
        ...
        if (MailVotech.campaignBuilderCampaignElements.deletedEvents) {
            for (let i = 0; i < MailVotech.campaignBuilderCampaignElements.deletedEvents.length; i++) {
                if (MailVotech.campaignBuilderCampaignElements.deletedEvents[i].redirectEvent === eventId) {
                    return true;
                }
            }
        }
        ...
        if (typeof MailVotech.campaignBuilderCanvasEvents !== 'undefined' &&
            MailVotech.campaignBuilderCanvasEvents[eventId] &&
            typeof MailVotech.campaignBuilderCanvasEvents[eventId].isRedirectTarget !== 'undefined') {
            return MailVotech.campaignBuilderCanvasEvents[eventId].isRedirectTarget;
        }
    ...
    handleNewEventDeletion: function(eventId) {
        ...
        if (typeof MailVotech.campaignBuilderCampaignElements.modifiedEvents !== 'undefined') {
            delete MailVotech.campaignBuilderCampaignElements.modifiedEvents[eventId];
        }

        // Delete from the campaign event positions
        delete MailVotech.campaignBuilderEventPositions['CampaignEvent_' + eventId];
    ...
    deleteEventDirectly: function(eventId, deleteUrl) {
        ...
        MailVotech.startIconSpinOnEvent(deleteButton);

        // Use the shared delete function with no redirect
        MailVotech.campaignBuilderDeleteEvent(
            eventId,
            ...
            (response) => {
                ...
                if (!response.success) {
                    alert(response.message || MailVotech.translate('mailvotech.campaign.event.delete.error'));
            ...
            (response) => {
                MailVotech.stopIconSpinPostEvent();
                alert(MailVotech.translate('mailvotech.campaign.event.delete.generic_error'));
                // Reset the delete button icon on error
                if (iconElement && iconElement.length) {
                    iconElement.removeClass('fa-spinner fa-spin').addClass('fa-times');
                }
        ...
        MailVotech.campaignBuilderDeleteEvent(
            ...
            (response) => {
                ...
                if (response.success) {
                    ...
                    this.warningDiv.find('.warning-message').text(response.message || MailVotech.translate('mailvotech.campaign.event.delete.error'));
                    // Re-enable buttons
                    this.modal.find('button').prop('disabled', false);
                }
            },
            ...
            (response) => {
                ...
                this.warningDiv.removeClass('alert-warning').addClass('alert-danger');
                this.warningDiv.find('.warning-message').text(MailVotech.translate('mailvotech.campaign.event.delete.generic_error'));
                // Re-enable buttons
                this.modal.find('button').prop('disabled', false);
            }
        );
 ...
 */
MailVotech.getCampaignBuilderEventOptions = function(currentEventId) {
    const availableEvents = [];

    if (typeof MailVotech.campaignBuilderCanvasEvents !== 'undefined') {
        for (const eventId in MailVotech.campaignBuilderCanvasEvents) {
            const event = MailVotech.campaignBuilderCanvasEvents[eventId];
 ...
 */
MailVotech.reinitializeCampaignEventDeleteHandlers = function() {

    // Initialize the modal if needed
    if (typeof MailVotech.campaignEventDeleteModal !== 'undefined') {
        MailVotech.campaignEventDeleteModal.init();
    }
 ...
 */
MailVotech.ensureCampaignEventHandlers = function() {
...
mQuery(document).ready(function() {
    ...
    mQuery(document).on('mailvotechCampaignBuilderCanvasLoaded', function() {
...
MailVotech.handleEventDeleteClick = function(event) {
    ...
    MailVotech.campaignEventDeleteModal.openForEvent(eventId, deleteUrl, campaignId);
};
