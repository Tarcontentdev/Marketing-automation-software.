...
final class MailVotechSsoFactory extends AbstractFactory implements AuthenticatorFactoryInterface
{
    /**
    ...
    {
        $authenticatorId = 'security.authenticator.mailvotech_sso.'.$firewallName;

        // use same auth handlers as in login_form
        $authenticationSuccessHandlerId = $this->getSuccessHandlerId($firewallName);
        $formLoginSuccessHandlerId      = str_replace('mailvotech_sso', 'form_login', $authenticationSuccessHandlerId);
        $authenticationFailureHandlerId = $this->getFailureHandlerId($firewallName);
        $formLoginFailureHandlerId      = str_replace('mailvotech_sso', 'form_login', $authenticationFailureHandlerId);

        $authenticator = $container
            ->setDefinition($authenticatorId, new ChildDefinition('security.authenticator.mailvotech_sso'))
            ->replaceArgument('$options', []) // Options will be replaced in \MailVotech\UserBundle\DependencyInjection\Compiler\SsoAuthenticatorPass
