...
namespace MailVotech\PluginBundle\Tests\DependencyInjection\Compiler;

use MailVotech\PluginBundle\Integration\AbstractIntegration;
use MailVotech\PluginBundle\Tests\Integration\ClientFactory;
...
final class TestPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container): void
    {
        $container->register(ClientFactory::class, ClientFactory::class)
            ->setArguments([new Reference('mailvotech.http.client')]);

        foreach ($container->getDefinitions() as $definition) {
            $class = (string) $definition->getClass();

            /** @phpstan-ignore-next-line  Ignore as AbstractIntegration is deprecated */
            if (str_starts_with($class, 'MailVotech') && is_subclass_of($class, AbstractIntegration::class)) {
