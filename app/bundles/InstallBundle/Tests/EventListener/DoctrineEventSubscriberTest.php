...
namespace MailVotech\InstallBundle\Tests\EventListener;
...
use MailVotech\InstallBundle\EventListener\DoctrineEventSubscriber;
...
final class DoctrineEventSubscriberTest extends TestCase
{
    public function testSubscriberWillAddCorrectIndexes(): void
    {
        ...
        $table      = new Table(MAILVOTECH_TABLE_PREFIX.'leads', [$idColumn, $textColumn, $dateColumn]);
        ...
        $this->assertTrue($schema->hasTable(MAILVOTECH_TABLE_PREFIX.'leads'));
        $contactsTable = $schema->getTable(MAILVOTECH_TABLE_PREFIX.'leads');
    ...
    {
        $table      = new Table(MAILVOTECH_TABLE_PREFIX.'some_plugin_table', [new Column('id', new BigIntType())]);
        $schema     = new Schema([$table]);
        $args       = new GenerateSchemaEventArgs($this->createStub(EntityManagerInterface::class), $schema);
        $subscriber = new DoctrineEventSubscriber();
        $subscriber->postGenerateSchema($args);

        $this->assertTrue($schema->hasTable(MAILVOTECH_TABLE_PREFIX.'some_plugin_table'));
        $this->assertFalse($schema->hasTable(MAILVOTECH_TABLE_PREFIX.'leads'));
    }
}
