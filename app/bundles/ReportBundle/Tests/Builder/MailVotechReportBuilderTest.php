...
namespace MailVotech\ReportBundle\Tests\Builder;
...
final class MailVotechReportBuilderTest extends TestCase
{
    ...
    public function testFiltersWithTag(): void
    {
        ...
        $this->assertSame(trim(preg_replace('/\s{2,}/', ' ', '
            SELECT `l`.`id`, `l`.`email` WHERE (l.id IN (SELECT DISTINCT lead_id FROM '.MAILVOTECH_TABLE_PREFIX.'lead_tags_xref ltx WHERE ltx.tag_id IN (1, 2))) AND (l.id NOT IN (SELECT DISTINCT lead_id FROM '.MAILVOTECH_TABLE_PREFIX.'lead_tags_xref ltx WHERE ltx.tag_id IN (3)))
        ')), $query->getSql());
    }
    ...
    public function testApplyTagFilter(): void
    {
        ...
        $this->assertSame('(l.id IN (SELECT DISTINCT lead_id FROM '.MAILVOTECH_TABLE_PREFIX.'lead_tags_xref ltx WHERE ltx.tag_id IN (1, 2))) AND (l.id NOT IN (SELECT DISTINCT lead_id FROM '.MAILVOTECH_TABLE_PREFIX.'lead_tags_xref ltx WHERE ltx.tag_id IN (3)))', $groupExpr->__toString());
        $this->assertNull($builder->getTagCondition($filters[2]));
    }

    private function buildBuilder(Report $report): MailVotechReportBuilder
    {
        return new MailVotechReportBuilder(
