...
namespace MailVotechPlugin\MailVotechFullContactBundle\Controller;
...
use MailVotechPlugin\MailVotechFullContactBundle\Helper\LookupHelper;
...
final class PublicController extends FormController
{
     ...
     */
    public function callbackAction(Request $request, LookupHelper $lookupHelper, LoggerInterface $mailvotechLogger): Response
    {
        ...
        if ('company' == $validatedRequest['type']) {
            return $this->compcallbackAction($mailvotechLogger, $result, $validatedRequest);
        }
        ...
        try {
            ...
            $mailvotechLogger->log('debug', 'SET FIELDS: '.print_r($data, true));
            ...
            if ($notify && (!$lead->imported)) {
                if ($user = $this->userModel->getEntity($notify)) {
                    $this->addNewNotification(
                        sprintf($this->translator->trans('mailvotech.plugin.fullcontact.contact_retrieved'), $lead->getEmail()),
                        'FullContact Plugin',
                        'ri-search-line',
                        $user
                    );
            ...
            try {
                if ($notify && $lead && (!$lead->imported)) {
                    if ($user = $this->userModel->getEntity($notify)) {
                        $this->addNewNotification(
                            sprintf(
                                $this->translator->trans('mailvotech.plugin.fullcontact.unable'),
                                $lead->getEmail(),
                                $ex->getMessage()
                            ),
                            'FullContact Plugin',
                ...
                $mailvotechLogger->log('error', 'FullContact: '.$ex2->getMessage());
            }
        }
     ...
     * @throws \InvalidArgumentException
     */
    private function compcallbackAction(LoggerInterface $mailvotechLogger, array $result, array $validatedRequest): Response
    {
        $notify = $validatedRequest['notify'];
        ...
        try {
            ...
            if ($notify) {
                if ($user = $this->userModel->getEntity($notify)) {
                    $this->addNewNotification(
                        sprintf($this->translator->trans('mailvotech.plugin.fullcontact.company_retrieved'), $company->getName()),
                        'FullContact Plugin',
                        'ri-search-line',
                        $user
                    );
