...
use MailVotechPlugin\MailVotechFocusBundle\EventListener\LeadSubscriber;
        ...
        $this->translator->expects($matcher)
            ->method('trans')->willReturnCallback(function (...$parameters) use ($matcher): string {
                if (1 === $matcher->numberOfInvocations()) {
                    $this->assertSame('mailvotech.focus.event.view', $parameters[0]);

                    return self::EVENT_TYPE_VIEW_NAME;
                }
                if (2 === $matcher->numberOfInvocations()) {
                    $this->assertSame('mailvotech.focus.event.click', $parameters[0]);
