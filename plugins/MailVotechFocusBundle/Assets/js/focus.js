//@ sourceURL=/plugins/MailVotechFocusBundle/Assets/js/focus.js

MailVotech.disabledFocusActions = function(opener) {
...
MailVotech.focusOnLoad = function () {
    if (mQuery('.builder').length) {
        ...
        mQuery('.btn-fetch').on('click', function () {
            ...
            if (url) {
                ...
                MailVotech.launchFocusBuilder();
            } else {
                return;
            }
        });

        MailVotech.focusInitViewportSwitcher();
    } else {
        MailVotech.initDateRangePicker();
    }

    if (mQuery('[data-conversion-rate-table]').length) {
        MailVotech.focusLoadConversionRateTable();
    }
    else {
        MailVotech.focusLoadViewCountTable();
    }
};

MailVotech.launchFocusBuilder = function (forceFetch) {
    mQuery('body').css('overflow-y', 'hidden');

    // Prevent preview updates till the website snapshot is loaded
    MailVotech.ignoreMailVotechFocusPreviewUpdate = true;
    ...
    if (!url) {
        if (!mQuery('#focus_unlockId').val()) {
            MailVotech.setFocusDefaultColors();
        }
        ...
        MailVotech.loadedPreviewImage = url;
        ...
        MailVotech.focusCreateIframe(url);

        MailVotech.ignoreMailVotechFocusPreviewUpdate = false;
    }
};

// Called when you  click on the show builder button
MailVotech.focusUpdatePreview = function () {

    // Generate a preview
    var data = mQuery('form[name=focus]').formToArray();
    MailVotech.ajaxActionRequest('plugin:focus:generatePreview', data, function (response) {
        ...
        if (!mQuery('.mf-bar').length && mQuery('.builder-content').length) {
            mQuery('.builder-content').on('click', function () {
                MailVotech.closeFocusModal(mQuery('#focus_style').val());
            });
            mQuery('.mailvotech-focus').on('click', function (e) {
                e.stopPropagation();
            });
        }
    });
...
MailVotech.setFocusDefaultColors = function () {
...
MailVotech.toggleBarCollapse = function () {
...
MailVotech.closeFocusModal = function (style) {
...
MailVotech.closeFocusBuilder = function (el) {
    // Kill preview updates
    if (typeof MailVotech.ajaxActionXhr != 'undefined' && typeof MailVotech.ajaxActionXhr['plugin:focus:generatePreview'] != 'undefined') {
        MailVotech.ajaxActionXhr['plugin:focus:generatePreview'].abort();
        delete MailVotech.ajaxActionXhr['plugin:focus:generatePreview'];
    }
...
MailVotech.focusInitViewportSwitcher = function () {
    mQuery('.btn-viewport').on('click', function () {
        if (mQuery(this).data('viewport') == 'mobile') {
            ...
            MailVotech.launchFocusBuilder(true);
        } else {
 ...
 */
MailVotech.focusCreateIframe = function (url) {
...
MailVotech.focusLoadConversionRateTable = function() {
    ...
    MailVotech.ajaxActionRequest('plugin:focus:getViewsCount', {focusId: focusId}, function(response){
    ...
    MailVotech.ajaxActionRequest('plugin:focus:getClickThroughCount', {focusId: focusId}, function(response){
        clickThrough = response.clickThrough;
        updateTotalClickThroughRate();
    }, false, true, "GET");
}

MailVotech.focusLoadViewCountTable = function() {
