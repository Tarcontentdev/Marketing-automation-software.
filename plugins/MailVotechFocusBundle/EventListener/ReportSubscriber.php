...
final class ReportSubscriber implements EventSubscriberInterface
{
    ...
    public function onReportBuilder(ReportBuilderEvent $event): void
    {
        ...
        $commonColumns = [
            ...
            self::PREFIX_STATS.'.conversion_rate_submission' => [
                'label'   => 'mailvotech.report.focus.ratio.submission',
                'type'    => 'string',
                'suffix'  => ' %',
                    ...
                    WHEN '.self::PREFIX_STATS.'.type = "submission" THEN (
                        SELECT
                            ROUND(
                                ...
                                NULLIF((
                                    SELECT COUNT(fs3.id)
                                    FROM '.MAILVOTECH_TABLE_PREFIX.'focus_stats fs3
                                    WHERE fs3.type = "view"
            ...
            self::PREFIX_STATS.'.conversion_rate_click' => [
                'label'   => 'mailvotech.report.focus.ratio.click',
                'type'    => 'string',
                'suffix'  => ' %',
        ...
        $statsColumns = [self::PREFIX_TRACKABLES.'.unique_hits' => [
            'label'   => 'mailvotech.report.focus.uniquehits',
            'type'    => 'int',
        ...
        $data = [
            'display_name' => 'mailvotech.focus.graph.stats',
    ...
    private function addFocusLeadsTable(ReportBuilderEvent $event, array $columns): void
    {
        ...
        $data = [
            'display_name' => 'mailvotech.report.datasource.focus.leads',
    ...
    public function onReportGenerate(ReportGeneratorEvent $event): void
    {
        ...
        $queryBuilder->from(MAILVOTECH_TABLE_PREFIX.'focus_stats', self::PREFIX_STATS)
            ->leftJoin(self::PREFIX_STATS, MAILVOTECH_TABLE_PREFIX.'focus', self::PREFIX_FOCUS,
                self::PREFIX_FOCUS.'.id = '.self::PREFIX_STATS.'.focus_id')
            ->leftJoin(self::PREFIX_STATS, MAILVOTECH_TABLE_PREFIX.'channel_url_trackables', self::PREFIX_TRACKABLES,
                self::PREFIX_TRACKABLES.'.channel_id = '.self::PREFIX_STATS.'.focus_id AND '.
                self::PREFIX_TRACKABLES.'.channel = "focus"')
            ->leftJoin(self::PREFIX_STATS, MAILVOTECH_TABLE_PREFIX.'page_redirects', self::PREFIX_REDIRECTS,
        ...
        if ($event->hasColumn(self::PREFIX_FOCUS.'.category')) {
            $queryBuilder->leftJoin(self::PREFIX_FOCUS, MAILVOTECH_TABLE_PREFIX.'categories', self::PREFIX_CATEGORIES,
                self::PREFIX_FOCUS.'.category_id = '.self::PREFIX_CATEGORIES.'.id');
        }

        switch ($event->getContext()) {
            case self::CONTEXT_FOCUS_LEADS:
                $queryBuilder->leftJoin(self::PREFIX_FOCUS, MAILVOTECH_TABLE_PREFIX.'leads', self::PREFIX_LEADS,
