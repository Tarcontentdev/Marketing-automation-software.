...
final class PublicController extends FormController
{
     ...
     */
    public function callbackAction(Request $request, LoggerInterface $mailvotechLogger, LookupHelper $lookupHelper): Response
    {
        if (!$request->request->has('body') || !$request->request->has('id')
            ...
            $mailvotechLogger->log('error', 'ERROR on Clearbit callback: Malformed request variables: '.json_encode($request->request->all(), JSON_PRETTY_PRINT));
        ...
        if (!$validatedRequest || !is_array($result)) {
            $mailvotechLogger->log('error', 'ERROR on Clearbit callback: Wrong body or id in request: id='.$oid.' body='.json_encode($result, JSON_PRETTY_PRINT));
        ...
        try {
            if ('person' === $request->request->get('type')) {
                ...
                $mailvotechLogger->log('debug', 'CURRFIELDS: '.var_export($currFields, true));
                ...
                $mailvotechLogger->log('debug', 'SETTING FIELDS: '.print_r($data, true));
                ...
                if ($notify && (!$lead->imported)) {
                    if ($user = $this->userModel->getEntity($notify)) {
                        $this->addNewNotification(
                            sprintf($this->translator->trans('mailvotech.plugin.clearbit.contact_retrieved'), $lead->getEmail()),
                            'Clearbit Plugin',
                            'ri-search-line',
                            $user
                        );
                ...
                if ('company' === $request->request->get('type')) {
                    ...
                    if ($notify) {
                        if ($user = $this->userModel->getEntity($notify)) {
                            $this->addNewNotification(
                                sprintf($this->translator->trans('mailvotech.plugin.clearbit.company_retrieved'), $company->getName()),
                                'Clearbit Plugin',
                                'ri-search-line',
                                $user
                            );
            ...
            $mailvotechLogger->log('error', 'ERROR on Clearbit callback: '.$ex->getMessage());
            try {
                if ($notify) {
                    if ($user = $this->userModel->getEntity($notify)) {
                        $this->addNewNotification(
                            sprintf(
                                $this->translator->trans('mailvotech.plugin.clearbit.unable'),
                                $ex->getMessage()
                            ),
                            'Clearbit Plugin',
                ...
                $mailvotechLogger->log('error', 'Clearbit: '.$ex2->getMessage());
            }
        }
