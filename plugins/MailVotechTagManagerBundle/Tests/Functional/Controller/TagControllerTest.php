...
namespace MailVotechPlugin\MailVotechTagManagerBundle\Tests\Functional\Controller;
...
final class TagControllerTest extends MailVotechMysqlTestCase
{
    ...
    private function countLeadTagAssociations(int $tagId): int
    {
        return (int) $this->em->getConnection()->createQueryBuilder()
            ->select('COUNT(*)')
            ->from(MAILVOTECH_TABLE_PREFIX.'lead_tags_xref')
            ->where('tag_id = :tagId')
