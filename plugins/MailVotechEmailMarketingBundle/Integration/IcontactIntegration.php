...
namespace MailVotechPlugin\MailVotechEmailMarketingBundle\Integration;

use MailVotechPlugin\MailVotechEmailMarketingBundle\Form\Type\IcontactType;
...
final class IcontactIntegration extends EmailAbstractIntegration
{
    ...
    {
        return [
            'API-AppId'    => 'mailvotech.icontact.keyfield.appid',
            'API-Username' => 'mailvotech.icontact.keyfield.username',
            'API-Password' => 'mailvotech.icontact.keyfield.password',
        ];
    }
    ...
    public function getAvailableLeadFields(array $settings = []): array
    {
        ...
        if (empty($leadFields)) {
            ...
            foreach ($fields as $f) {
                $leadFields[$f] = [
                    'label'    => $this->translator->trans('mailvotech.icontact.field.'.$f),
                    'type'     => 'string',
                    'required' => 'email' === $f,
                ];
            }
