...
use MailVotechPlugin\MailVotechEmailMarketingBundle\Form\Type\ConstantContactType;
...
final class ConstantContactIntegration extends EmailAbstractIntegration
{
    ...
    public function getAvailableLeadFields(array $settings = []): array
    {
        ...
        foreach ($fields as $f) {
            $leadFields[$f] = [
                'label'    => $this->translator->trans('mailvotech.constantcontact.field.'.$f),
                'type'     => 'string',
                'required' => 'email' === $f,
            ];
        }
        ...
        while ($c <= 15) {
            $leadFields['customfield_'.$c] = [
                'label'    => $this->translator->trans('mailvotech.constantcontact.customfield.'.$f),
                'type'     => 'string',
                'required' => false,
            ];
            ++$c;
