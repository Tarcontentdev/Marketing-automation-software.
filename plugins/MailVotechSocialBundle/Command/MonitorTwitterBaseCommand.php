...
namespace MailVotechPlugin\MailVotechSocialBundle\Command;
...
use MailVotechPlugin\MailVotechSocialBundle\Event\SocialMonitorEvent;
use MailVotechPlugin\MailVotechSocialBundle\Helper\TwitterCommandHelper;
use MailVotechPlugin\MailVotechSocialBundle\Integration\TwitterIntegration;
...
abstract class MonitorTwitterBaseCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (false === $twitterIntegration || false === $twitterIntegration->getIntegrationSettings()->getIsPublished()) {
            $this->output->writeln($this->translator->trans('mailvotech.social.monitoring.twitter.not.published'));
        ...
        if (!$this->twitter->isAuthorized()) {
            $this->output->writeln($this->translator->trans('mailvotech.social.monitoring.twitter.not.configured'));
        ...
        if (!$mid) {
            $this->output->writeln($this->translator->trans('mailvotech.social.monitoring.twitter.mid.empty'));
        ...
        if (!$monitor || !$monitor->getId()) {
            $this->output->writeln($this->translator->trans('mailvotech.social.monitoring.twitter.monitor.does.not.exist', ['%id%' => $mid]));
    ...
    protected function processMonitor($monitor)
    {
        ...
        if (count($results['statuses'])) {
            ...
            $this->output->writeln($this->translator->trans('mailvotech.social.monitoring.twitter.no.new.tweets'));
        }
