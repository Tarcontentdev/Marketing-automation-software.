...
final class JsController extends CommonController
{
    ...
    function postAuthCallback(response){
        var elements = document.getElementById("mailvotechform_{$formName}").elements;
        var field, fieldName;
        values = JSON.parse(JSON.stringify(response));
        
        for (var i = 0, element; element = elements[i++];) {
            field = element.name
            fieldName = field.replace("mailvotechform[","");
            fieldName = fieldName.replace("]","");
            var element = document.getElementsByName("mailvotechform["+fieldName+"]");
