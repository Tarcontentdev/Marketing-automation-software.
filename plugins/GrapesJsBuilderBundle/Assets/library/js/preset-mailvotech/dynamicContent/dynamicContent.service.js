...
export default class DynamicContentService {
   ...
   * Creates a store item (filter) in MailVotech Form if new.
   * Wires up the ids.
  ...
  updateDcStoreItem() {
    ...
    // so it has to be moved back to the MailVotech email form for saving.
    const modalContent = mQuery('#dynamic-content-popup');
    // On modal close -> move editor within MailVotech
    if (!modalContent) {
  ...
  createNewStoreItem(component) {
    const storeItemIdentifier = MailVotech.createNewDynamicContentItem(mQuery);
