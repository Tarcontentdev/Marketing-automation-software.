...
export default class DynamicContentCommands {
  ...
  stopDynamicContentPopup() {
    ...
    if (
      ...
      MailVotech.internalDynamicContentItemCreateListeners.length
    ) {
      MailVotech.internalDynamicContentItemCreateListeners = [];
    }
    if (
      MailVotech.internalDynamicContentFilterCreateListeners &&
      MailVotech.internalDynamicContentFilterCreateListeners.length
    ) {
      MailVotech.internalDynamicContentFilterCreateListeners = [];
    }
  ...
  showDynamicContentPopup(editor, sender, options) {
    ...
    const title = MailVotech.translate('grapesjsbuilder.dynamicContentBlockLabel');
    ...
    MailVotech.setDynamicContentEditors(MailVotech.getBuilderContainer());

    // When a new Dynamic Content filter (tab) is added, we want to turn the editor into CKEditor.
    MailVotech.dynamicContentAddNewFilterListener((textarea) => {
      MailVotech.ConvertFieldToCkeditor(textarea, {});
    });

    // When a new Dynamic Content item (slot) is added, we want to turn the editor into CKEditor.
    MailVotech.dynamicContentAddNewItemListener((textarea) => {
      MailVotech.ConvertFieldToCkeditor(textarea, {});
    });

    modal.onceClose(() => editor.stopCommand('preset-mailvotech:dynamic-content-open'));
  }
  ...
  /**
   * Delete DynamicContent on MailVotech side
   *
   * @param component
   */
  // eslint-disable-next-line class-methods-use-this
