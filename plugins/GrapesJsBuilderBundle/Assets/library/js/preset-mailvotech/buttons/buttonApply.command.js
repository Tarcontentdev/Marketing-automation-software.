...
export default class ButtonApplyCommand {
   ...
   * The command name
   */
  static name = 'preset-mailvotech:apply-form';
   ...
   * Use the global MailVotech postForm function
   *
   * @param editor
   * @param sender
   */
  static postForm(editor, sender) {
    const mailvotechForm = ButtonsService.getMailVotechForm();

    sender.set('className', 'fa fa-spinner fa-spin');

    ButtonApplyCommand.setDefaultValues(editor);

    MailVotech.inBuilderSubmissionOn(mailvotechForm);
    MailVotech.postForm(mailvotechForm, ButtonApplyCommand.postFormResponse.bind(this, editor, sender));
    MailVotech.inBuilderSubmissionOff();
  }

  /**
   * Get and handle response
   * Use the global MailVotech functions
   *
   * @param editor
   * @param sender
   * @param response
  ...
  static postFormResponse(editor, sender, response) {
    ...
    if (response.validationError !== null) {
      const title = MailVotech.translate('grapesjsbuilder.panelsViewsCommandModalTitleError');
      ButtonApplyCommand.showModal(editor, title, response.validationError);
    } else {
      if (response.route) {
        // update URL in address bar
        MailVotechVars.manualStateChange = false;
      ...
      // update form action
      if (mailvotechForm[0].baseURI !== mailvotechForm[0].action) {
        mailvotechForm[0].action = mailvotechForm[0].baseURI;
      }
    }
