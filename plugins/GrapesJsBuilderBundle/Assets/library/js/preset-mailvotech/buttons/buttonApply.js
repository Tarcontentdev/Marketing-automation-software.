...
export default class ButtonApply {
  ...
  addButton() {
    ...
    let title = MailVotech.translate('grapesjsbuilder.panelsViewsButtonsApplyTitle');
    let disable = false;
    ...
    if (mode === ContentService.modeEmailHtml || mode === ContentService.modeEmailMjml) {
      ...
      if (emailType === emailTypeSegment) {
        ...
        if (emailFormList.length === 0) {
          title = MailVotech.translate('grapesjsbuilder.panelsViewsButtonsApplyTitleError');
          disable = true;
          command = '';
        }
      }
