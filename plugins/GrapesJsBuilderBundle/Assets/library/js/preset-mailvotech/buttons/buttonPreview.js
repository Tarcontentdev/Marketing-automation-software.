...
export default class ButtonPreview {
   ...
   */
  addButton() {
    let title = MailVotech.translate('grapesjsbuilder.buttons.buttonPreview.title');
    let disable = false;
    let command = ButtonPreview.getCommand();

    if (ButtonsService.isNewEntity()) {
      title = MailVotech.translate('grapesjsbuilder.buttons.buttonPreview.titleDisabled');
      disable = true;
      command = '';
    }
