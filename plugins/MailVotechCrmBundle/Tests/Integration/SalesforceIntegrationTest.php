...
namespace MailVotechPlugin\MailVotechCrmBundle\Tests\Integration;
...
use MailVotechPlugin\MailVotechCrmBundle\Integration\SalesforceIntegration;
...
final class SalesforceIntegrationTest extends AbstractIntegrationTestCase
{
    ...
    public const SC_MULTIPLE_MAILVOTECH_CONTACTS = 'multiple_mailvotech_contacts';
     ...
     */
    private array $mailvotechContacts = [
        'Contact' => [],
        'Lead'    => [],
    ];
    ...
    public function testThatLeadsAreOnlyCreatedIfEnabled(): void
    {
        ...
        $this->sfMockMethods = ['makeRequest', 'getMailVotechContactsToCreate'];

        $sf = $this->getSalesforceIntegration();

        $sf->expects($this->never())
            ->method('getMailVotechContactsToCreate');
    ...
    {
        $this->sfMockMethods = ['makeRequest', 'getMailVotechContactsToCreate', 'getMailVotechContactsToUpdate', 'getSalesforceSyncLimit'];

        $sf      = $this->getSalesforceIntegration();
        $counter = 0;
        $sf->expects($this->exactly(2))
            ->method('getMailVotechContactsToUpdate')
            ->willReturnCallback(
    ...
    public function testLastSyncDate(): void
    {
        ...
        // should be teh same as MAILVOTECH_DATE_MODIFIED_OVERRIDE override
        $this->assertInstanceOf(\DateTime::class, $lastSync);
        $this->assertEquals(MAILVOTECH_DATE_MODIFIED_OVERRIDE, $lastSync->format('U'));
    ...
    public function testLeadsAreNotCreatedInSfIfFoundToAlreadyExistAsContacts(): void
    {
        ...
        $this->sfMockMethods = ['makeRequest', 'getSalesforceObjectsByEmails', 'prepareMailVotechContactsToCreate'];
        ...
        $sf->expects($this->never())
            ->method('prepareMailVotechContactsToCreate');
    ...
    public function testExportingContactActivity(): void
    {
        ...
        /*
         * Ensures that makeRequest is called with the mailvotech_timeline__c endpoint.
    ...
    public function testMailVotechContactTimelineLinkPopulatedsPayload(): void
    {
        $this->sfObjects     = ['Contact'];
        $this->sfMockMethods = ['makeRequest', 'getSalesforceObjectsByEmails'];
        $sf                  = $this->getSalesforceIntegration(2, 2);
        ...
        /*
         * When checking if contacts need to be created, the mailvotechContactTimelineLink
            ...
            ->with(
                ...
                [
                    'contact1@sftest.com' => [
                        ...
                        'mailvotechContactTimelineLink'         => 'mailvotech_plugin_timeline_view',
                        'mailvotechContactIsContactableByEmail' => 1,
                        'mailvotechContactId'                   => 1,
                    ],
                    'contact2@sftest.com' => [
        ...
        foreach ($objects as $object) {
            $mappedData = [
                ...
                'contact2@sftest.com' => [
                    ...
                    'email'                             => 'Contact2@sftest.com',
                    'mailvotechContactTimelineLink'         => 'mailvotech_plugin_timeline_view',
                    'mailvotechContactIsContactableByEmail' => 0,
                ],
            ];
            ...
            foreach ($mappedData as $assertion) {
                $this->assertArrayHasKey('mailvotechContactIsContactableByEmail', $assertion);
            }
        }
    }

    public function testUpdateDncByMailVotechDate(): void
    {
        ...
        foreach ($objects as $object) {
            ...
            $sf = $this->getSalesforceIntegration(2, 2);
            $sf->method('updateDncByDate')->willReturn(true);
            $sf->method('getDoNotContactHistory')->willReturn($this->getSalesforceDNCHistory($object, 'MailVotech'));

            $sf->pushLeadDoNotContactByDate('email', $mappedData, $object, ['start' => '2017-10-15T10:00:00.000000']);
            foreach ($mappedData as $assertion) {
                $this->assertArrayNotHasKey('mailvotechContactIsContactableByEmail', $assertion);
            }
        }
    }
                    ...
                    switch (true) {
                        case str_contains($args[0], '/query'):
                            ...
                            if (isset($args[1]['q']) && str_contains($args[1]['q'], 'from '.$updateObject.'History')) {
                                return $this->getSalesforceDNCHistory($updateObject, 'MailVotech');
                            }
                            // Extract emails
    ...
    /**
     * Simulate looping over MailVotech leads to update.
     *
    ...
    protected function getLeadsToUpdate(string $object, int $limit, int $max, ?string $specificObject): array
    {
        ...
        $this->mailvotechContacts = array_merge($entities[$object], $this->mailvotechContacts);

        return $entities;
    }

    /**
     * Simulate looping over MailVotech leads to create.
     *
    ...
    protected function getLeadsToCreate(int $limit, int $max = 200): array
    {
        ...
        $this->mailvotechContacts = array_merge($entities, $this->mailvotechContacts);
    ...
    protected function getSalesforceDNCHistory(string $object, string $priority): array
    {
        $datePriority = [
            ...
            'MailVotech' => '2017-10-16T18:43:43.000+0000',
        ];
     ...
     */
    protected function getMailVotechContacts(): array
    {
        $contacts             = $this->mailvotechContacts;
        $this->mailvotechContacts = [
            'Contact' => [],
            'Lead'    => [],
        ];
