...
use MailVotechPlugin\MailVotechCrmBundle\Api\ConnectwiseApi;
use MailVotechPlugin\MailVotechCrmBundle\Form\Type\IntegrationCampaignsTaskType;
...
class ConnectwiseIntegration extends CrmAbstractIntegration
{
    ...
    {
        return [
            'username'  => 'mailvotech.connectwise.form.integrator',
            'password'  => 'mailvotech.connectwise.form.privatekey',
            'site'      => 'mailvotech.connectwise.form.site',
            'appcookie' => 'mailvotech.connectwise.form.cookie',
        ];
    }
    ...
    public function appendToForm(&$builder, $data, $formArea): void
    {
        if ('features' == $formArea) {
            ...
            $builder->add(
                ...
                [
                    'choices' => [
                        'mailvotech.connectwise.object.contact' => 'Contact',
                        'mailvotech.connectwise.object.company' => 'company',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'mailvotech.connectwise.form.objects_to_pull_from',
                    'label_attr'        => ['class' => ''],
                    'placeholder'       => false,
                    'required'          => false,
                ]
        ...
        if ('integration' == $formArea) {
            if ($this->isAuthorized()) {
                $builder->add(
                    ...
                    [
                        'label'      => 'mailvotech.plugin.config.push.activities',
                        'label_attr' => ['class' => 'control-label'],
                        'attr'       => [
                            'class' => 'form-control',
                        ],
    ...
    public function getRecords($params, $object): int
    {
        ...
        try {
            while ($records = ('Contact' == $object)
                ...
                $mailvotechReferenceObject = ('Contact' == $object) ? 'lead' : 'company';
                foreach ($records as $record) {
                    if (is_array($record)) {
                        $id            = $record['id'];
                        $formattedData = $this->amendLeadDataBeforeMailVotechPopulate($record, $object);
                        $entity        = ('Contact' == $object)
                            ? $this->getMailVotechLead($formattedData)
                            : $this->getMailVotechCompany(
                                $formattedData,
                                'company'
                            );
                        if ($entity) {
                            $integrationEntities[] = $this->saveSyncedData($entity, $object, $mailvotechReferenceObject, $id);
    ...
    /**
     * Ammend mapped lead data before creating to MailVotech.
     *
     * @return mixed[]
     */
    public function amendLeadDataBeforeMailVotechPopulate($data, $object): array
    {
        $fieldsValues = [];
    ...
    public function saveSyncedData($entity, $object, $mailvotechObjectReference, $integrationEntityId): IntegrationEntity
    {
    ...
    {
        $fieldsToUpdateInCW = isset($config['update_mailvotech']) && $personFound ? array_keys($config['update_mailvotech'], 1) : [];
        $objectFields       = $this->prepareFieldsForPush($this->getContactFields());
        $leadFields         = $config['leadFields'];

        $cwContactExists = $this->amendLeadDataBeforeMailVotechPopulate($cwContactData, $object);
    ...
    public function populateLeadData($lead, $config = []): array
    {
        ...
        foreach ($availableFields as $key => $field) {
            ...
            if ('communicationItems' == $integrationKey) {
                ...
                foreach ($field['items']['keys'] as $keyItem => $item) {
                    ...
                    if (isset($leadFields[$item])) {
                        ...
                        $mailvotechKey = $leadFields[$item];
                        if (isset($fields[$mailvotechKey]) && !empty($fields[$mailvotechKey]['value'])) {
                            foreach ($config['communicationItems'] as $key => $ci) {
                                if ($ci['type']['id'] == $keyItem + 1) {
                                    $config['communicationItems'][$key]['value'] = $fields[$mailvotechKey]['value'];
                                    $keyExists                                   = true;
                                }
                            }
                            if (!$keyExists) {
                                ...
                                $values = array_merge(['value' => $this->cleanPushData($fields[$mailvotechKey]['value'])], $defaultValue);
            ...
            if (isset($leadFields[$integrationKey])) {
                ...
                if (isset($fields[$mailvotechKey]) && !empty($fields[$mailvotechKey]['value'])) {
                    $matched[$matchIntegrationKey] = $this->cleanPushData($fields[$mailvotechKey]['value']);
                }
            }
    ...
    {
        if ('company' == $object) {
            $priority = parent::getPriorityFieldsForMailVotech($config, $object, 'mailvotech_company');
    ...
    public function saveCampaignMembers($allCampaignMembers, IntegrationObject $campaignMemberObject, $campaignId): void
    {
        ...
        $mailvotechObject    = new IntegrationObject('Contact', 'lead');

        $contacts = $this->integrationEntityModel->getSyncedRecords($mailvotechObject, $this->getName(), $recordList);
