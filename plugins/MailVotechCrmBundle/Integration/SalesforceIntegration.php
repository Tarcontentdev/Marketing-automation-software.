...
namespace MailVotechPlugin\MailVotechCrmBundle\Integration;
...
use MailVotech\CoreBundle\Entity\Transformer\NotificationArrayTransformer;
...
use MailVotechPlugin\MailVotechCrmBundle\Api\SalesforceApi;
use MailVotechPlugin\MailVotechCrmBundle\Integration\Salesforce\CampaignMember\Fetcher;
use MailVotechPlugin\MailVotechCrmBundle\Integration\Salesforce\CampaignMember\Organizer;
use MailVotechPlugin\MailVotechCrmBundle\Integration\Salesforce\Exception\NoObjectsToFetchException;
use MailVotechPlugin\MailVotechCrmBundle\Integration\Salesforce\Helper\StateValidationHelper;
use MailVotechPlugin\MailVotechCrmBundle\Integration\Salesforce\Object\CampaignMember;
use MailVotechPlugin\MailVotechCrmBundle\Integration\Salesforce\ResultsPaginator;
...
class SalesforceIntegration extends CrmAbstractIntegration
{
    ...
    {
        return [
            'client_id'     => 'mailvotech.integration.keyfield.consumerid',
            'client_secret' => 'mailvotech.integration.keyfield.consumersecret',
        ];
    }
        ...
        try {
            if (!empty($salesForceObjects) && is_array($salesForceObjects)) {
                foreach ($salesForceObjects as $sfObject) {
                    if ('Account' === $sfObject) {
                        // Match SF object to MailVotech's
                        $sfObject = 'company';
                    }
                            ...
                            if (!empty($fields['fields'])) {
                                foreach ($fields['fields'] as $fieldInfo) {
                                    ...
                                    if ('company' !== $sfObject) {
                                        ...
                                        // CreateDate can be updatable just in MailVotech
                                        if ('CreatedDate' == $fieldInfo['name']) {
                                            $salesFields[$sfObject][$fieldInfo['name'].'__'.$sfObject]['update_mailvotech'] = 1;
                                        }
                                    } else {
    ...
    public function getFormNotes($section)
    {
        if ('authorization' == $section) {
            return ['mailvotech.salesforce.form.oauth_requirements', 'warning'];
        }
     ...
     */
    public function amendLeadDataBeforeMailVotechPopulate($data, $object, $params = []): array
    {
        $updated               = 0;
        $created               = 0;
        $counter               = 0;
        $entity                = null;
        $detachClass           = null;
        $mailvotechObjectReference = null;
        $integrationMapping    = [];
        ...
        if (isset($data['records']) && 'Activity' !== $object) {
            foreach ($data['records'] as $record) {
                $this->logger->debug('SALESFORCE: amendLeadDataBeforeMailVotechPopulate record '.var_export($record, true));
                ...
                if ([] !== $dataObject) {
                    ...
                    switch ($object) {
                        case 'Contact':
                            if (isset($dataObject['Email__Contact'])) {
                                ...
                                // correctly against mailvotech emails
                        ...
                        case 'Lead':
                            ...
                            if (isset($dataObject['Email__Lead'])) {
                                // Sanitize email to make sure we match it
                                // correctly against mailvotech_leads emails
                            ...
                            $entity                = $this->getMailVotechLead($dataObject, true, null, null, $object);
                            $mailvotechObjectReference = 'lead';
                            $detachClass           = Lead::class;
                            break;
                        case 'Account':
                            $entity                = $this->getMailVotechCompany($dataObject, 'Account');
                            $mailvotechObjectReference = 'company';
                    ...
                    if ($counter >= 100) {
                        ...
                        $this->buildIntegrationEntities($integrationMapping, $object, $mailvotechObjectReference, $params);
                        $counter = 0;
            ...
            unset($data['records']);
            $this->logger->debug('SALESFORCE: amendLeadDataBeforeMailVotechPopulate response '.var_export($data, true));
    ...
    public function appendToForm(&$builder, $data, $formArea): void
    {
        if ('features' == $formArea) {
            $builder->add(
                ...
                [
                    'choices' => [
                        'mailvotech.salesforce.sandbox' => 'sandbox',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'mailvotech.salesforce.form.sandbox',
                    'label_attr'        => ['class' => 'control-label'],
                    'placeholder'       => false,
                    'required'          => false,
                    'attr'              => [
                        'onclick' => 'MailVotech.postForm(mQuery(\'form[name="integration_details"]\'),\'\');',
                    ],
                ]
            );
            ...
            $builder->add(
                ...
                [
                    'choices' => [
                        'mailvotech.salesforce.updateOwner' => 'updateOwner',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'mailvotech.salesforce.form.updateOwner',
                    'label_attr'        => ['class' => 'control-label'],
                    'placeholder'       => false,
                    'required'          => false,
                    'attr'              => [
            ...
            $builder->add(
                ...
                [
                    'choices' => [
                        'mailvotech.integrations.blanks' => 'updateBlanks',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'mailvotech.integrations.form.blanks',
                    'label_attr'        => ['class' => 'control-label'],
                    'placeholder'       => false,
                    'required'          => false,
                ]
            ...
            $builder->add(
                ...
                [
                    'choices' => [
                        'mailvotech.integrations.update.dnc.by.date' => 'updateDncByDate',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'mailvotech.integrations.form.update.dnc.by.date.label',
                    'label_attr'        => ['class' => 'control-label'],
                    'placeholder'       => false,
                    'required'          => false,
                ]
            ...
            $builder->add(
                ...
                [
                    'choices' => [
                        'mailvotech.salesforce.object.lead'     => 'Lead',
                        'mailvotech.salesforce.object.contact'  => 'Contact',
                        'mailvotech.salesforce.object.company'  => 'company',
                        'mailvotech.salesforce.object.activity' => 'Activity',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'mailvotech.salesforce.form.objects_to_pull_from',
                    'label_attr'        => ['class' => ''],
                    'placeholder'       => false,
                    'required'          => false,
                ]
            ...
            $builder->add(
                ...
                [
                    ...
                    'label'             => 'mailvotech.salesforce.form.activity_included_events',
                    'label_attr'        => [
                        'class'       => 'control-label',
                        'data-toggle' => 'tooltip',
                        'title'       => $this->translator->trans('mailvotech.salesforce.form.activity.events.tooltip'),
                    ],
                    'multiple'   => true,
            ...
            $builder->add(
                ...
                TextType::class,
                [
                    'label'      => 'mailvotech.salesforce.form.namespace_prefix',
                    'label_attr' => ['class' => 'control-label'],
        ...
        try {
            if ($this->isAuthorized()) {
                ...
                while (true) {
                    ...
                    [$justUpdated, $justCreated] = $this->amendLeadDataBeforeMailVotechPopulate($result, $object, $params);
    ...
    public function pushLeads(array $params = []): array
    {
        ...
        [$fieldMapping, $mailvotechLeadFieldString, $supportedObjects] = $this->prepareFieldsForPush($config);
        ...
        if (defined('IN_MAILVOTECH_CONSOLE')) {
            // start with update
        ...
        while ($totalCount > 0) {
            ...
            // Process the updates
            if (!$noMoreUpdates) {
                $noMoreUpdates = $this->getMailVotechContactsToUpdate(
                    $checkEmailsInSF,
                    $mailvotechLeadFieldString,
                    $sfObject,
                    $trackedContacts,
            ...
            // If there is still room - grab MailVotech leads to create if the Lead object is enabled
            $sfEntityRecords = [];
            if ('Lead' === $sfObject && ($limit > 0) && !empty($mailvotechLeadFieldString)) {
                try {
                    $sfEntityRecords = $this->getMailVotechContactsToCreate(
                        $checkEmailsInSF,
                        $fieldMapping,
                        $mailvotechLeadFieldString,
                        $limit,
            ...
            $this->prepareMailVotechContactsToUpdate(
                $mailvotechData,
                $checkEmailsInSF,
                $processedLeads,
                $trackedContacts,
            ...
            if ($checkEmailsInSF && isset($fieldMapping['Lead'])) {
                $this->prepareMailVotechContactsToCreate(
                    $mailvotechData,
                    $checkEmailsInSF,
                    $processedLeads,
                    $fieldMapping
            ...
            // Make the request
            $this->makeCompositeRequest($mailvotechData, $totalUpdated, $totalCreated, $totalErrors);
    ...
    public function getCampaignMembers($campaignId): void
    {
        ...
        while (true) {
            try {
                ...
                // Create MailVotech contacts from Campaign Members if they don't already exist
                foreach (['Contact', 'Lead'] as $object) {
                ...
                foreach ($unknownMembers as $mailvotechContactId) {
    ...
    public function pushLeadToCampaign(Lead $lead, $campaignId, $status = '', $personIds = null): bool
    {
        ...
        if (!empty($lead->getEmail())) {
            ...
            $request['compositeRequest'] = array_values($mailvotechData);

            $this->logger->debug('SALESFORCE: pushLeadToCampaign '.var_export($request, true));

            $result = $this->getApiHelper()->syncMailVotechToSalesforce($request);
    ...
    protected function getMailVotechContactsToUpdate(
        &$checkEmailsInSF,
        $mailvotechLeadFieldString,
        &$sfObject,
        ...
        foreach ($toUpdate as $lead) {
            if (!empty($lead['email'])) {
                $lead                                               = $this->getCompoundMailVotechFields($lead);
     ...
     * @throws ApiErrorException
     */
    protected function getMailVotechContactsToCreate(
        &$checkEmailsInSF,
        array $fieldMapping,
        $mailvotechLeadFieldString,
        $limit,
        ...
        // For any MailVotech contacts left over, check to see if existing Leads exist
    ...
    protected function buildCompositeBody(
        ...
        if ((isset($entity['email']) && !empty($entity['email'])) || (isset($entity['companyname']) && !empty($entity['companyname']))) {
            ...
            foreach ($fields as $sfField => $mailvotechField) {
                if (isset($entity[$mailvotechField])) {
                    $fieldType = (isset($objectFields['types']) && isset($objectFields['types'][$sfField])) ? $objectFields['types'][$sfField]
                        : 'string';
                    if (!empty($entity[$mailvotechField]) && 'boolean' != $fieldType) {
                        $body[$sfField] = $this->cleanPushData($entity[$mailvotechField], $fieldType);
                ...
                if (array_key_exists($sfField, $objectFields['required']['fields']) && empty($body[$sfField])) {
                    if (isset($sfRecord[$sfField])) {
                        ...
                        if (empty($entity[$mailvotechField]) && !empty($sfRecord[$sfField])
                            && $sfRecord[$sfField] !== $this->translator->trans(
                                'mailvotech.integration.form.lead.unknown'
                            )
                        ) {
                            $updateEntity[$mailvotechField] = $sfRecord[$sfField];
                        }
                    } else {
                        $body[$sfField] = $this->translator->trans('mailvotech.integration.form.lead.unknown');
                    }
                }
            }
    ...
    protected function prepareFieldsForPush($config): array
    {
        ...
        unset($leadFields['mailvotechContactTimelineLink']);
        unset($leadFields['mailvotechContactIsContactableByEmail']);
        ...
        $mailvotechLeadFieldString = implode(', l.', $leadFields);
        $mailvotechLeadFieldString = 'l.'.$mailvotechLeadFieldString;
        ...
        return [$objectFields, $mailvotechLeadFieldString, $supportedObjects];
    }
     ...
     * @return mixed
     */
    protected function getPriorityFieldsForMailVotech($config, $object = null, $priorityObject = 'mailvotech')
    {
        $fields = parent::getPriorityFieldsForMailVotech($config, $object, $priorityObject);
     ...
     * @return mixed
     */
    protected function getPriorityFieldsForIntegration($config, $object = null, $priorityObject = 'mailvotech')
    {
    ...
    protected function prepareMailVotechContactsToUpdate(
        &$mailvotechData,
        ...
        foreach ($sfEntityRecords['records'] as $sfKey => $sfEntityRecord) {
            ...
            if (
                ...
                if ($contactEntity = $this->checkLeadIsContact($trackedContacts['Contact'], $key, $contactId, $mailvotechLeadFieldString)) {
            ...
            if (!$skipObject) {
                ...
                // Keep track of MailVotech ID to Salesforce ID for the integration table
                ...
                if ($updateLead = $this->buildCompositeBody(
                    ...
                    // Get the lead entity
                    foreach ($updateLead as $mailvotechField => $sfValue) {
                        /** @var Lead $leadEntity */
                        $leadEntity->addUpdatedField($mailvotechField, $sfValue);
                    }

                    $syncLead = !empty($leadEntity->getChanges(true));
                }

                // Validate if we have a company for this MailVotech contact
    ...
    protected function prepareMailVotechContactsToCreate(
        &$mailvotechData,
        array &$checkEmailsInSF,
        array &$processedLeads,
        array $objectFields,
     ...
     * @param int $totalErrored
     */
    protected function makeCompositeRequest($mailvotechData, &$totalUpdated = 0, &$totalCreated = 0, &$totalErrored = 0): void
    {
        if (empty($mailvotechData)) {
            return;
        }
        ...
        $request              = [];
        $request['allOrNone'] = 'false';
        $chunked              = array_chunk($mailvotechData, 25);

        foreach ($chunked as $chunk) {
            // We can only submit 25 at a time
            if ([] !== $chunk) {
                $request['compositeRequest'] = $chunk;
                $result                      = $apiHelper->syncMailVotechToSalesforce($request);
    ...
    protected function setContactToSync(array &$checkEmailsInSF, array $lead): false|string
    {
        ...
        if (isset($checkEmailsInSF[$key])) {
            // this is a duplicate in MailVotech
            $this->mailvotechDuplicates[$lead['internal_entity_id']] = 'lead-duplicate';
    ...
    protected function mapCompanyDataForPush(Company $company, array $config): array
    {
        ...
        if (isset($config['objects']) && in_array($object, $config['objects'])) {
            ...
            $fieldsToUpdateInSf = $this->getPriorityFieldsForIntegration($config, 'Account', 'mailvotech_company');
    ...
    public function pushLeadDoNotContactByDate($channel, &$sfRecords, $sfObject, array $params = []): void
    {
        ...
        if (empty($sfRecords) || !isset($sfRecords['mailvotechContactIsContactableByEmail']) && !$this->updateDncByDate()) {
            return;
        }
        ...
        if (count($DNCCreatedContacts)) {
            $this->updateMailVotechDNC($DNCCreatedContacts, true);
        }
        ...
        // get last modified date for donot contact in MailVotech
        ...
        foreach ($historySF['records'] as $sfModifiedDNC) {
            // if we have no history in MailVotech, then update the MailVotech record
            if (empty($lastModifiedDNCDate)) {
                $leads  = array_flip($leadIds);
                $leadId = $leads[$sfModifiedDNC[$sfObject.'Id']];
                $this->updateMailVotechDNC($leadId, $sfModifiedDNC['NewValue']);
                $key = $this->getSyncKey($leadEmails[$leadId]);
                unset($sfRecords[$key]['mailvotechContactIsContactableByEmail']);
                continue;
            }
            ...
            foreach ($lastModifiedDNCDate as $logs) {
                ...
                if ((isset($leadIds[$leadId]) && $leadIds[$leadId] == $sfModifiedDNC[$sfObject.'Id'])
                    ...
                    // SF was updated last so update MailVotech record
                    $key = $this->getSyncKey($leadEmails[$leadId]);
                    unset($sfRecords[$key]['mailvotechContactIsContactableByEmail']);
                    $this->updateMailVotechDNC($leadId, $sfModifiedDNC['NewValue']);
                    $trackedIds[] = $leadId;
     ...
     */
    private function updateMailVotechDNC($leadId, $newDncValue): void
    {
    ...
    public function pushCompanies(array $params = []): array
    {
        ...
        // get company fields from MailVotech that have been mapped
        $mailvotechCompanyFieldString = 'l.'.implode(', l.', $config['companyFields']);

        $fieldKeys          = array_keys($config['companyFields']);
        $fieldsToCreate     = $this->prepareFieldsForSync($config['companyFields'], $fieldKeys, $sfObject);
        $fieldsToUpdateInSf = $this->getPriorityFieldsForIntegration($config, $sfObject, 'mailvotech_company');
        ...
        while ($totalCount > 0) {
            ...
            // Process the updates
            if (!$noMoreUpdates) {
                $noMoreUpdates = $this->getMailVotechRecordsToUpdate(
                    $checkCompaniesInSF,
                    $mailvotechCompanyFieldString,
                    $sfObject,
                    $limit,
            ...
            // If there is still room - grab MailVotech companies to create if the Lead object is enabled
            $sfEntityRecords = [];
            if ($limit > 0) {
                $this->getMailVotechEntitesToCreate(
                    $checkCompaniesInSF,
                    $mailvotechCompanyFieldString,
                    $limit,
                    $fromDate,
            ...
            if ([] !== $sfEntityRecords && isset($sfEntityRecords['records'])) {
                $this->prepareMailVotechCompaniesToUpdate(
                    $mailvotechData,
                    $checkCompaniesInSF,
                    $processedCompanies,
                    $companiesToSync,
            ...
            // Only create left over if Lead object is enabled in integration settings
            if ([] !== $checkCompaniesInSF) {
                $this->prepareMailVotechCompaniesToCreate(
                    $mailvotechData,
                    $checkCompaniesInSF,
                    $processedCompanies,
                    $objectFields
    ...
    protected function prepareMailVotechCompaniesToUpdate(
        &$mailvotechData,
        ...
        foreach ($sfEntityRecords['records'] as $sfEntityRecord) {
            ...
            if (isset($checkCompaniesInSF[$key])) {
                ...
                foreach ($checkCompaniesInSF as $mailvotechKey => $mailvotechCompanies) {
                    $key = $mailvotechKey;

                    if (isset($mailvotechCompanies['companyname']) && $mailvotechCompanies['companyname'] == $sfEntityRecord['Name']) {
                        $companyData = $processedCompanies[$key] ?? $checkCompaniesInSF[$key];
            ...
            if ($updateCompany = $this->buildCompositeBody(
                ...
                foreach ($updateCompany as $mailvotechField => $sfValue) {
                    /** @var Company $companyEntity */
                    $companyEntity->addUpdatedField($mailvotechField, $sfValue);
                }
    ...
    protected function prepareMailVotechCompaniesToCreate(
        &$mailvotechData,
        array &$checkCompaniesInSF,
        array &$processedCompanies,
        array $objectFields,
    ...
    protected function getMailVotechRecordsToUpdate(
        array &$checkIdsInSF,
        $mailvotechEntityFieldString,
        &$sfObject,
        $limit,
        $fromDate,
        $toDate,
    ...
    protected function getMailVotechEntitesToCreate(
        array &$checkIdsInSF,
        $mailvotechCompanyFieldString,
        $limit,
        $fromDate,
    ...
    {
        if (isset($matchedFields['mailvotechContactIsContactableByEmail']) && $this->updateDncByDate()) {
