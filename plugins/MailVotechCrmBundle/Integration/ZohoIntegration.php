...
use MailVotechPlugin\MailVotechCrmBundle\Api\Zoho\Mapper;
use MailVotechPlugin\MailVotechCrmBundle\Api\ZohoApi;
...
final class ZohoIntegration extends CrmAbstractIntegration
{
    ...
    {
        return [
            $this->getClientIdKey()     => 'mailvotech.zoho.form.client_id',
            $this->getClientSecretKey() => 'mailvotech.zoho.form.client_secret',
        ];
    }
     ...
     * @param string $object
     */
    public function amendLeadDataBeforeMailVotechPopulate($data, $object = null): array
    {
        if ('company' === $object) {
            $object = 'Accounts';
            ...
            foreach ($objects as $recordId => $entityData) {
                ...
                if ('Accounts' === $object) {
                    ...
                    if (count($integrationId)) { // company exists, then update local fields
                        ...
                        $matchedFields = $this->populateMailVotechLeadData($entityData, $config, 'company');

                        // Match that data with mapped lead fields
                        $fieldsToUpdateInMailVotech = $this->getPriorityFieldsForMailVotech($config, $object, 'mailvotech_company');
                        if (!empty($fieldsToUpdateInMailVotech)) {
                            $fieldsToUpdateInMailVotech = array_intersect_key($config['companyFields'], $fieldsToUpdateInMailVotech);
                            $newMatchedFields       = array_intersect_key($matchedFields, array_flip($fieldsToUpdateInMailVotech));
                        } else {
                        ...
                        $entity = $this->getMailVotechCompany($entityData, 'Accounts');
                    }
                    if ($entity) {
                    ...
                    if (count($integrationId)) { // lead exists, then update
                        ...
                        $matchedFields = $this->populateMailVotechLeadData($entityData, $config, $object);

                        // Match that data with mapped lead fields
                        $fieldsToUpdateInMailVotech = $this->getPriorityFieldsForMailVotech($config, $object, 'mailvotech');

                        if (!empty($fieldsToUpdateInMailVotech)) {
                            $fieldsToUpdateInMailVotech = array_intersect_key($config['leadFields'], $fieldsToUpdateInMailVotech);
                            $newMatchedFields       = array_intersect_key($matchedFields, array_flip($fieldsToUpdateInMailVotech));
                        } else {
                            $newMatchedFields = $matchedFields;
                        }
                        ...
                        /** @var Lead $entity */
                        $entity = $this->getMailVotechLead($entityData, true, null, null, $object);
                    }
        ...
        try {
            if ($this->isAuthorized()) {
                ...
                while (true) {
                    ...
                    $result   = $this->amendLeadDataBeforeMailVotechPopulate($data, $object);
     ...
     * @param array $config
     */
    public function populateMailVotechLeadData($data, $config = [], $object = 'Leads'): array
    {
        // Match that data with mapped lead fields
        $aFields       = $this->getAvailableLeadFields($config);
        $matchedFields = [];
    ...
    public function appendToForm(&$builder, $data, $formArea): void
    {
        ...
        if ('keys' === $formArea) {
            $builder->add(
                ...
                [
                    'choices' => [
                        'mailvotech.plugin.zoho.zone_us'     => 'zoho.com',
                        'mailvotech.plugin.zoho.zone_europe' => 'zoho.eu',
                        'mailvotech.plugin.zoho.zone_japan'  => 'zoho.co.jp',
                        'mailvotech.plugin.zoho.zone_china'  => 'zoho.com.cn',
                    ],
                    'label'       => 'mailvotech.plugin.zoho.zone_select',
                    'placeholder' => false,
                    'required'    => true,
                    'attr'        => [
                        'tooltip' => 'mailvotech.plugin.zoho.zone.tooltip',
                    ],
                ]
            );
            ...
            $builder->add(
                ...
                [
                    'choices' => [
                        'mailvotech.zoho.object.lead'    => 'Leads',
                        'mailvotech.zoho.object.contact' => 'Contacts',
                        'mailvotech.zoho.object.account' => 'company',
                    ],
                    'expanded'    => true,
                    'multiple'    => true,
                    'label'       => $this->getTranslator()->trans('mailvotech.crm.form.objects_to_pull_from', ['%crm%' => 'Zoho']),
                    'label_attr'  => ['class' => ''],
                    'placeholder' => false,
                    'required'    => false,
                ]
    ...
    public function pushLeads(array $params = []): array
    {
        ...
        $config                = $this->mergeConfigToFeatureSettings();
        $fieldsToUpdateInZoho  = isset($config['update_mailvotech']) ? array_keys($config['update_mailvotech'], 0) : [];
        $leadFields            = array_unique(array_values($config['leadFields']));
        $totalUpdated          = $totalCreated = $totalErrors = 0;
        if ($key = array_search('mailvotechContactTimelineLink', $leadFields)) {
            unset($leadFields[$key]);
        }
        if ($key = array_search('mailvotechContactIsContactableByEmail', $leadFields)) {
            unset($leadFields[$key]);
        }
        if ([] === $leadFields) {
            return [0, 0, 0];
     ...
     *
     * @throws \MailVotechPlugin\MailVotechCrmBundle\Api\Zoho\Exception\MatchingKeyNotFoundException
     */
    private function consumeResponse($response, string $zObject, bool $createIntegrationEntity = false, ?Mapper $mapper = null): int
    {
        ...
        foreach ($rows as $key => $row) {
            $mailvotechId = $mapper->getContactIdByKey($key);
            ...
            if ('SUCCESS' === $row['code'] && $createIntegrationEntity) {
                ...
                if (0 === count($integrationId)) {
                    $this->createIntegrationEntity($zObject, $zohoId, 'lead', $mailvotechId);
                }
            } elseif (isset($row['status']) && 'error' === $row['status']) {
                ++$failed;
                $exception = new ApiErrorException($row['message']);
                $exception->setContactId($mailvotechId);
                $this->logIntegrationError($exception);
            }
        }
