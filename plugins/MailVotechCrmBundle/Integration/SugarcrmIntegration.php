...
use MailVotechPlugin\MailVotechCrmBundle\Api\SugarcrmApi;
...
final class SugarcrmIntegration extends CrmAbstractIntegration
{
    ...
    {
        return $this->router->generate('mailvotech_integration_auth_callback', ['integration' => $this->getName()]);
    }
    ...
    {
        return [
            'sugarcrm_url'  => 'mailvotech.sugarcrm.form.url',
            'client_id'     => 'mailvotech.sugarcrm.form.clientkey',
            'client_secret' => 'mailvotech.sugarcrm.form.clientsecret',
            'username'      => 'mailvotech.sugarcrm.form.username',
            'password'      => 'mailvotech.sugarcrm.form.password',
        ];
    }
        ...
        try {
            if (is_array($sugarObjects)) {
                foreach ($sugarObjects as $sObject) {
                    if ('Accounts' === $sObject) {
                        // Match Sugar object to MailVotech's
                        $sObject = 'company';
                    }
    ...
    public function getCompanies(array $params = [], $query = null, $executed = null): int|float|null
    {
        ...
        try {
            if ($this->isAuthorized()) {
                ...
                $executed += $this->amendLeadDataBeforeMailVotechPopulate($result, $sugarObject);
                if (
        ...
        try {
            if ($this->isAuthorized()) {
                if ('Activity' !== $object && 'company' !== $object) {
                    ...
                    $params['offset'] = $result['next_offset'];
                    $executed += $this->amendLeadDataBeforeMailVotechPopulate($result, $object);
                    if (
                        (isset($result['total_count']) && $result['total_count'] > $params['offset'])   // Sugar 6
                        || (!isset($result['total_count']) && $params['offset'] > -1)) {            // Sugar 7
    ...
    public function getErrorsFromResponse($response)
    {
        if ('6' == $this->keys['version']) {
            ...
            return $this->translator->trans('mailvotech.integration.error.genericerror', [], 'flashes');
        }
    ...
    /**
     * Amend mapped lead data before creating to MailVotech.
     *
     * @param array  $data
     * @param string $object
     */
    public function amendLeadDataBeforeMailVotechPopulate($data, $object): int
    {
            ...
            foreach ($data[$RECORDS_LIST_NAME] as $record) {
                ...
                if ('6' === $SUGAR_VERSION) {
                    foreach ($record['name_value_list'] as $item) {
                        if ($this->checkIfSugarCrmMultiSelectString($item['value'])) {
                            $convertedMultiSelectString         = $this->convertSuiteCrmToMailVotechMultiSelect($item['value']);
                ...
                if ([] !== $dataObject) {
                    if ('Leads' == $object || 'Contacts' == $object) {
                        ...
                        $company               = null;
                        $this->fetchDncToMailVotech($entity, $data);
                        ...
                        $entity                = $this->getMailVotechCompany($dataObject, $object);
                    ...
                    if ($entity) {
                        ...
                        if (null == $integrationId) {
                            ...
                            $integrationEntity->setInternalEntity($mailvotechObjectReference);
    ...
    public function appendToForm(&$builder, $data, $formArea): void
    {
        if ('keys' == $formArea) {
            $builder->add('version', ButtonGroupType::class, [
                ...
                'label'             => 'mailvotech.sugarcrm.form.version',
                'constraints'       => [
        ...
        if ('features' == $formArea) {
            $builder->add(
                ...
                [
                    'choices' => [
                        'mailvotech.sugarcrm.updateOwner' => 'updateOwner',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'mailvotech.sugarcrm.form.updateOwner',
            ...
            $builder->add(
                ...
                [
                    'choices' => [
                        'mailvotech.sugarcrm.updateDnc' => 'updateDnc',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'mailvotech.sugarcrm.form.updateDnc',
                    'label_attr'        => ['class' => 'control-label'],
                    'placeholder'       => false,
                    'required'          => false,
                    'attr'              => [
            ...
            $builder->add(
                ...
                [
                    'choices' => [
                        'mailvotech.sugarcrm.object.lead'    => 'Leads',
                        'mailvotech.sugarcrm.object.contact' => 'Contacts',
                        'mailvotech.sugarcrm.object.company' => 'company',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'mailvotech.sugarcrm.form.objects_to_pull_from',
                    'label_attr'        => ['class' => ''],
                    'placeholder'       => false,
                    'required'          => false,
                ]
    ...
    public function pushLead($lead, $config = []): array|bool
    {
        ...
        $fieldsToUpdateInSugar      = isset($config['update_mailvotech']) ? array_keys($config['update_mailvotech'], 0) : [];
    ...
    public function pushLeads(array $params = []): array
    {
        ...
        $limit                   = $params['limit'];
        $config                  = $this->mergeConfigToFeatureSettings();
        $mailvotechData              = $leadsToUpdate              = $fields              = [];
        $fieldsToUpdateInSugar   = isset($config['update_mailvotech']) ? array_keys($config['update_mailvotech'], 0) : [];
        $leadFields              = $config['leadFields'];
        if (!empty($leadFields)) {
            if ($keys = array_keys($leadFields, 'mailvotechContactTimelineLink')) {
                foreach ($keys as $key) {
                    unset($leadFields[$key]);
                }
            }

            if ($keys = array_keys($leadFields, 'mailvotechContactIsContactableByEmail')) {
                foreach ($keys as $key) {
                    unset($leadFields[$key]);
                }
            }
        ...
        foreach ($checkEmailsInSugar as $object => $checkObjectEmailsInSugar) {
            [$checkEmailsUpdatedInSugar, $deletedRedords] = $this->getObjectDataToUpdate($checkObjectEmailsInSugar, $mailvotechData, $availableFields, $contactSugarFields, $leadSugarFields, $object);
        ...
        if ($checkEmailsInSugar && isset($checkEmailsInSugar['Leads'])) {
            [$checkEmailsInSugar, $deletedSugarLeads]     = $this->getObjectDataToUpdate($checkEmailsInSugar['Leads'], $mailvotechData, $availableFields, $contactSugarFields, $leadSugarFields, 'Leads');
        ...
        $apiHelper = $this->getApiHelper();
        if ([] !== $mailvotechData) {
            $result = $apiHelper->syncLeadsToSugar($mailvotechData);
        }
    ...
    private function pushDncToSugar(array $lead, array &$body): void
    {
        ...
        /** @var \MailVotech\LeadBundle\Entity\DoNotContact[] $dncEntries */
    ...
    private function fetchDncToMailVotech(?Lead $lead = null, array $data = []): void
    {
        if (null === $lead) {
            return;
        }
        ...
        $reason = \MailVotech\LeadBundle\Entity\DoNotContact::UNSUBSCRIBED;
        if (!$isContactable) {
     ...
     * @param string $object
     *
     * @return array The first element is made up of records that exist in MailVotech, but which no longer have a match in CRM.
     ...
     */
    public function getObjectDataToUpdate($checkEmailsInSugar, array &$mailvotechData, $availableFields, $contactSugarFields, $leadSugarFields, $object = 'Leads'): array
    {
        $config     = $this->mergeConfigToFeatureSettings([]);
        ...
        foreach ($sugarLeadRecords as $sugarLeadRecord) {
            if (isset($sugarLeadRecord) && $sugarLeadRecord) {
                ...
                if (!empty($checkEmailsInSugar)) {
                    foreach ($checkEmailsInSugar as $emailKey => $mailvotechRecord) {
                        if ($key == $emailKey) {
                            ...
                            $lead['owner_email']                                             = $this->getOwnerEmail($mailvotechRecord);
                            if ($lead['owner_email']) {
    ...
    private function getOwnerEmail(array $lead)
    {
        if (isset($lead['owner_id']) && !empty($lead['owner_id'])) {
            /** @var \MailVotech\UserBundle\Entity\User $user */
    ...
    private function buildCompositeBody(array &$mailvotechData, array $availableFields, $fieldsToUpdateInSugarUpdate, string $object, array $lead, $onwerAssignedUserIdByEmail = null, $objectId = null): void
    {
        $body = [];
        if (isset($lead['email']) && !empty($lead['email'])) {
            // update and create (one query) every 200 records

            foreach ($fieldsToUpdateInSugarUpdate as $sugarField => $mailvotechField) {
                $required = !empty($availableFields[$object][$sugarField.'__'.$object]['required']);
                if (isset($lead[$mailvotechField])) {
                    if (str_contains($lead[$mailvotechField], '|')) {
                        // Transform MailVotech Multi Select into SugarCRM/SuiteCRM Multi Select format
                        $value = $this->convertMailVotechToSuiteCrmMultiSelect($lead[$mailvotechField]);
                    } else {
                        $value = $lead[$mailvotechField];
                    }
                    $body[] = ['name' => $sugarField, 'value' =>  $value];
                } elseif ($required) {
                    $value  = $this->translator->trans('mailvotech.integration.form.lead.unknown');
                    $body[] = ['name' => $sugarField, 'value' => $value];
                }
            }
    ...
    /**
     * Converts MailVotech Multi-Select String into the format used to store Multi-Select values used by SuiteCRM / SugarCRM 6.x.
     */
    public function convertMailVotechToSuiteCrmMultiSelect($mailvotechMultiSelectStringToConvert): string
    {
        // $mailvotechMultiSelectStringToConvert = 'test|enhancedapi|dataservices';
        $multiSelectArrayValues             = explode('|', $mailvotechMultiSelectStringToConvert);
    ...
    /**
     * Converts a SuiteCRM / SugarCRM 6.x Multi-Select String into the format used to store Multi-Select values used by MailVotech.
     */
    public function convertSuiteCrmToMailVotechMultiSelect($suiteCrmMultiSelectStringToConvert): string
    {
        // MailVotech Multi Select format - 'choice1|choice2|choice_3'
