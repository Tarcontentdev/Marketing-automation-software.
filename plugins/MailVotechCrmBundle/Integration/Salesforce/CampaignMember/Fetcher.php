...
namespace MailVotechPlugin\MailVotechCrmBundle\Integration\Salesforce\CampaignMember;

use MailVotech\PluginBundle\Entity\IntegrationEntityRepository;
use MailVotechPlugin\MailVotechCrmBundle\Integration\Salesforce\Exception\InvalidObjectException;
...
use MailVotechPlugin\MailVotechCrmBundle\Integration\Salesforce\Object\Contact;
use MailVotechPlugin\MailVotechCrmBundle\Integration\Salesforce\Object\Lead;
use MailVotechPlugin\MailVotechCrmBundle\Integration\Salesforce\QueryBuilder;
...
final class Fetcher
{
    ...
    /**
     * Fetch the MailVotech contact IDs that are not already tracked as SF campaign members.
     */
    public function getUnknownCampaignMembers(): array
    {
        ...
        $mailvotechLeadIds = array_map(
            fn (array $entity) => $entity['internal_entity_id'],
            $this->knownCampaignMembers
        );

        return array_values(array_diff($this->mailvotechIds, $mailvotechLeadIds));
    }
    ...
    private function fetchLeads(): void
    {
        ...
        foreach ($this->leads as $lead) {
            ...
            $this->mailvotechIds[]    = $lead['internal_entity_id'];
        }
    ...
    private function fetchContacts(): void
    {
        ...
        foreach ($this->contacts as $contact) {
            ...
            $this->mailvotechIds[]       = $contact['internal_entity_id'];
        }
