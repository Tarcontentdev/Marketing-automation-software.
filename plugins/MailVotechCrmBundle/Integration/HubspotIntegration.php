...
use MailVotechPlugin\MailVotechCrmBundle\Api\HubspotApi;
...
class HubspotIntegration extends CrmAbstractIntegration
{
                        ...
                        if (isset($leadFields)) {
                            foreach ($leadFields as $fieldInfo) {
                                ...
                                if (!empty($fieldInfo['readOnlyValue'])) {
                                    $hubsFields[$object][$fieldInfo['name']]['update_mailvotech'] = 1;
    ...
    public function appendToForm(&$builder, $data, $formArea): void
    {
        if ('keys' === $formArea) {
            $builder->add(
                ...
                TextType::class,
                [
                    'label'       => 'mailvotech.hubspot.form.accessKey',
            ...
            $builder->add(
                ...
                TextType::class,
                [
                    'label'       => 'mailvotech.hubspot.form.apikey',
                    'label_attr'  => ['class' => 'control-label'],
                    'attr'        => [
                        'class'    => 'form-control',
                        'readonly' => true,
        ...
        if ('features' == $formArea) {
            $builder->add(
                ...
                [
                    'choices' => [
                        'mailvotech.hubspot.object.contact' => 'contacts',
                        'mailvotech.hubspot.object.company' => 'company',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => $this->getTranslator()->trans('mailvotech.crm.form.objects_to_pull_from', ['%crm%' => 'Hubspot']),
                    'label_attr'        => ['class' => ''],
                    'placeholder'       => false,
                    'required'          => false,
                ]
            ...
            if ($this->isAuthorized()) {
                ...
                if (isset($data['contacts'])) {
                    foreach ($data['contacts'] as $contact) {
                        if (is_array($contact)) {
                            $contactData = $this->amendLeadDataBeforeMailVotechPopulate($contact, 'Lead');
                            $contact     = $this->getMailVotechLead($contactData);
            ...
            if ($this->isAuthorized()) {
                ...
                foreach ($results['results'] as $company) {
                    if (isset($company['properties'])) {
                        $companyData = $this->amendLeadDataBeforeMailVotechPopulate($company, null);
                        $company     = $this->getMailVotechCompany($companyData);
                        if ($id) {
                            return $company;
                        }
                        if ($company) {
    ...
    public function getMailVotechLead($data, $persist = true, $socialCache = null, $identifiers = null, $object = null)
    {
        ...
        if ($lead = parent::getMailVotechLead($data, false, $socialCache, $identifiers, $object)) {
            if (isset($stageName)) {
