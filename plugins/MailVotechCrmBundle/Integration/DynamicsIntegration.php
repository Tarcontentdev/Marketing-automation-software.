...
use MailVotechPlugin\MailVotechCrmBundle\Api\DynamicsApi;
...
final class DynamicsIntegration extends CrmAbstractIntegration
{
    ...
    {
        return [
            'resource'      => 'mailvotech.integration.dynamics.resource',
            'client_id'     => 'mailvotech.integration.dynamics.client_id',
            'client_secret' => 'mailvotech.integration.dynamics.client_secret',
        ];
    }
    ...
    public function appendToForm(&$builder, $data, $formArea): void
    {
        ...
        if ('features' === $formArea) {
            $builder->add(
                ...
                [
                    'choices' => [
                        'mailvotech.dynamics.object.contact'  => 'contacts',
                        'mailvotech.dynamics.object.company'  => 'company',
                    ],
                    'expanded'          => true,
                    'multiple'          => true,
                    'label'             => 'mailvotech.dynamics.form.objects_to_pull_from',
                    'label_attr'        => ['class' => ''],
                    'placeholder'       => false,
                    'required'          => false,
                ]
    ...
    public function getFormNotes($section): array
    {
        if ('custom' === $section) {
            return [
                'template'   => '@MailVotechCrm/Integration/dynamics.html.twig',
                'parameters' => [
                ],
            ];
        }
                ...
                if ('accounts' === $object) {
                    ...
                    if (count($integrationId)) { // company exists, then update local fields
                        ...
                        if (!empty($fieldsToUpdateInMailVotech)) {
                            $fieldsToUpdateInMailVotech = array_intersect_key($config['companyFields'], array_flip($fieldsToUpdateInMailVotech));
                        ...
                        $entity = $this->getMailVotechCompany($entityData, 'company');
                    }
                    if ($entity) {
                    ...
                    if (count($integrationId)) { // lead exists, then update
                        ...
                        $matchedFields = $this->populateMailVotechLeadData($entityData, $config);

                        // Match that data with mapped lead fields
                        $fieldsToUpdateInMailVotech = $this->getPriorityFieldsForMailVotech($config, $object, 'mailvotech');
                        if (!empty($fieldsToUpdateInMailVotech)) {
                            $fieldsToUpdateInMailVotech = array_intersect_key($config['leadFields'] ?? [], array_flip($fieldsToUpdateInMailVotech));
                            $newMatchedFields       = array_intersect_key($matchedFields, array_flip($fieldsToUpdateInMailVotech));
                        } else {
                            $newMatchedFields = $matchedFields;
                        }
                        ...
                        /** @var Lead $entity */
                        $entity = $this->getMailVotechLead($entityData);
                    }
    ...
    public function pushLeads(array $params = []): array
    {
        ...
        $object                = 'contacts';
        $config                = $this->mergeConfigToFeatureSettings();
        $fieldsToUpdateInCrm   = isset($config['update_mailvotech']) ? array_keys($config['update_mailvotech'], 0) : [];
        ...
        foreach ($leadsToUpdateInD as $lead) {
            $mappedData = [];
            if (defined('IN_MAILVOTECH_CONSOLE') && $progress) {
                $progress->advance();
            }
