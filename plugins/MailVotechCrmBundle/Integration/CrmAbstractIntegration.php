...
use MailVotechPlugin\MailVotechCrmBundle\Api\CrmApi;
...
abstract class CrmAbstractIntegration extends AbstractIntegration
{
    ...
    public function getLeads($params, $query, &$executed, $result = [], $object = 'Lead')
    {
        ...
        try {
            if ($this->isAuthorized()) {
                ...
                return $this->amendLeadDataBeforeMailVotechPopulate($result, $object);
            }
        } catch (\Exception $e) {
            $this->logIntegrationError($e);
        }
     ...
     */
    public function amendLeadDataBeforeMailVotechPopulate($data, $object)
    {
        return null;
    }
    ...
    public function getApiHelper()
    {
        if (empty($this->helper)) {
            $class        = '\\MailVotechPlugin\\MailVotechCrmBundle\\Api\\'.$this->getName().'Api';
     ...
     * @return Company|null
     */
    public function getMailVotechCompany($data, $object = null)
    {
        if (is_object($data)) {
        ...
        $config        = $this->mergeConfigToFeatureSettings([]);
        $matchedFields = $this->populateMailVotechLeadData($data, $config, 'company');
        ...
        if (!empty($existingCompany[2])) {
            $fieldsToUpdate = $this->getPriorityFieldsForMailVotech($config, $object, 'mailvotech_company');
     ...
     * @return Lead|null
     */
    public function getMailVotechLead($data, $persist = true, $socialCache = null, $identifiers = null, $object = null)
    {
        if (is_object($data)) {
            // Convert to array in all levels
            $data = json_encode(json_decode($data, true));
        ...
        $matchedFields = $this->populateMailVotechLeadData($data, $config);
        ...
        if (count(array_diff_key($uniqueLeadFields, $matchedFields)) === count($uniqueLeadFields)) {
            ...
            $this->logger->debug('getMailVotechLead: No unique identifiers', [
        ...
        if (!$lead->isNewlyCreated()) {
            ...
            if (empty($fieldsToUpdateInMailVotech)) {
                $this->logger->debug('getMailVotechLead: No fields to update in MailVotech', ['config' => $config, 'object' => $object]);

                return null;
            }

            $fieldsToUpdateInMailVotech = array_intersect_key($leadFields, $fieldsToUpdateInMailVotech);
            $matchedFields          = array_intersect_key($matchedFields, array_flip($fieldsToUpdateInMailVotech));
            if (isset($config['updateBlanks']) && isset($config['updateBlanks'][0]) && 'updateBlanks' == $config['updateBlanks'][0]) {
                $matchedFields = $this->getBlankFieldsToUpdateInMailVotech($matchedFields, $lead->getFields(true), $leadFields, $data, $object);
            }
        }
        ...
        if (isset($data['owner_email']) && isset($config['updateOwner']) && isset($config['updateOwner'][0])
            ...
            if ($mailvotechUser = $this->userRepository->findOneBy(['email' => $data['owner_email']])) {
                $lead->setOwner($mailvotechUser);
            }
        }
     ...
     * @return array
     */
    protected function getPriorityFieldsForMailVotech($config, $entityObject = null, $priorityObject = 'mailvotech')
    {
     ...
     * @return array
     */
    protected function getPriorityFieldsForIntegration($config, $entityObject = null, $priorityObject = 'mailvotech')
    {
        return $this->cleanPriorityFields(
    ...
    public function getBlankFieldsToUpdateInMailVotech($matchedFields, $leadFieldValues, $objectFields, $integrationData, $object = 'Lead')
    {
        foreach ($objectFields as $integrationField => $mailvotechField) {
            if (isset($leadFieldValues[$mailvotechField]) && empty($leadFieldValues[$mailvotechField]['value']) && !empty($integrationData[$integrationField.'__'.$object]) && $this->translator->trans('mailvotech.integration.form.lead.unknown') !== $integrationData[$integrationField.'__'.$object]) {
                $matchedFields[$mailvotechField] = $integrationData[$integrationField.'__'.$object];
            }
        }
