...
use MailVotechPlugin\MailVotechCrmBundle\Api\Salesforce\Exception\RetryRequestException;
use MailVotechPlugin\MailVotechCrmBundle\Api\Salesforce\Helper\RequestUrl;
...
final class SalesforceApi extends CrmApi
{
     ...
     * @throws ApiErrorException
     */
    public function syncMailVotechToSalesforce(array $data)
    {
    ...
    public function createLeadActivity(array $activity, $object): array
    {
        ...
        $mActivityObjectName = $namespace.'mailvotech_timeline__c';
        $activityData        = [];
            ...
            foreach ($activity as $sfId => $records) {
                foreach ($records['records'] as $record) {
                    $body = [
                        ...
                        $namespace.'MailVotech_url__c'   => $records['leadUrl'],
            ...
            if ([] !== $activityData) {
                ...
                foreach ($chunked as $chunk) {
                    ...
                    if ([] !== $chunk) {
                        ...
                        $result                      = $this->syncMailVotechToSalesforce($request);
                        $results[]                   = $result;
    ...
    public function getLeads($query, $object)
    {
        ...
        if (defined('MAILVOTECH_ENV') && MAILVOTECH_ENV === 'dev') {
            // Easier for testing
    ...
    public function getCampaignMembers($campaignId, $modifiedSince = null, $queryUrl = null)
    {
        ...
        // MailVotech IntegrationEntity objects
        $this->requestSettings['headers']['Sforce-Query-Options'] = 'batchSize=200';
        ...
        try {
            ...
            if ('HasOptedOutOfEmail' == $missingField) {
                ...
                $this->integration->upsertUnreadAdminsNotification(
                    $this->integration->getTranslator()->trans('mailvotech.salesforce.error.opt-out_permission.header'),
                    $this->integration->getTranslator()->trans('mailvotech.salesforce.error.opt-out_permission.message')
                );
            } else {
