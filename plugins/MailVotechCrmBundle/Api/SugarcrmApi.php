...
use MailVotechPlugin\MailVotechCrmBundle\Integration\SugarcrmIntegration;
...
final class SugarcrmApi extends CrmApi
{
    ...
    public function createLead(array $fields, $lead)
    {
        $tokenData       = $this->integration->getKeys();
        $createdLeadData = [];
        // search for Sugar id in mailvotech records first to avoid making an API call
        if (is_object($lead)) {
        ...
        // $createdLeadData[] = $this->request('set_entry', $parameters, 'POST');
            ...
            if (!empty($sugarLeadRecords)) {
                foreach ($sugarLeadRecords as $sLeadRecord) {
                    ...
                    $fieldsToUpdateInSugar = isset($config['update_mailvotech']) ? array_keys($config['update_mailvotech'], 1) : [];
