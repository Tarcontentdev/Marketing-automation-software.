<?php

declare(strict_types=1);

namespace MailVotechPlugin\MailVotechCrmBundle\Api\Zoho;

use MailVotechPlugin\MailVotechCrmBundle\Api\Zoho\Exception\MatchingKeyNotFoundException;

final class Mapper
{
    private array $contact = [];

    private array $mappedFields = [];

    private $object;

    /**
     * @var array[]
     */
    private array $objectMappedValues = [];

    /**
     * Used to keep track of the key used to map contact ID with the response Zoho returns.
     */
    private int $objectCounter = 0;

    /**
     * Used to map contact ID with the response Zoho returns.
     */
    private array $contactMapper = [];

    public function __construct(
        private array $fields,
    ) {
    }

    public function setObject($object): static
    {
        $this->object = $object;

        return $this;
    }

    public function setContact(array $contact): static
    {
        $this->contact = $contact;

        return $this;
    }

    public function setMappedFields(array $fields): static
    {
        $this->mappedFields = $fields;

        return $this;
    }

    /**
     * @param int      $mailvotechContactId MailVotech Contact ID
     * @param int|null $zohoId          Zoho ID if known
     *
     * @return int If any single field is mapped, return 1 to count as one contact to be updated
     */
    public function map($mailvotechContactId, $zohoId = null): int
    {
        $mapped             = 0;
        $objectMappedValues = [];

        foreach ($this->mappedFields as $zohoField => $mailvotechField) {
            $field = $this->getField($zohoField);
            if ($field && isset($this->contact[$mailvotechField]) && $this->contact[$mailvotechField]) {
                $mapped   = 1;
                $apiField = $field['api_name'];
                $apiValue = $this->contact[$mailvotechField];

                $objectMappedValues[$apiField] = $apiValue;
            }

            if ($zohoId) {
                $objectMappedValues['id'] = $zohoId;
            }
        }

        $this->objectMappedValues[$this->objectCounter] = $objectMappedValues;
        $this->contactMapper[$this->objectCounter]      = $mailvotechContactId;

        ++$this->objectCounter;

        return $mapped;
    }

    public function getArray(): array
    {
        return $this->objectMappedValues;
    }

    /**
     * @param int $key
     *
     * @return int
     *
     * @throws MatchingKeyNotFoundException
     */
    public function getContactIdByKey($key)
    {
        if (isset($this->contactMapper[$key])) {
            return $this->contactMapper[$key];
        }

        throw new MatchingKeyNotFoundException();
    }

    /**
     * @return mixed
     */
    private function getField(int|string $fieldName)
    {
        return $this->fields[$this->object][$fieldName] ?? null;
    }
}
