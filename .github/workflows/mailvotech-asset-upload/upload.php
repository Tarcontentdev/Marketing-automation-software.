...
use MailVotech\Auth\ApiAuth;
use MailVotech\MailVotechApi;
...
// Verify the URL is from GitHub (required for MailVotech release process)
...
// Enforce the exact GitHub release asset path for the specified MailVotech version
if (empty($parsedUrl['path'])) {
    echo "Error: Remote asset URL is missing a path component\n";
    exit(1);
}

$expectedPath = '/mailvotech/mailvotech/releases/download/'.$mailvotechVersion.'/'.$mailvotechVersion.'.zip';
if ($parsedUrl['path'] !== $expectedPath) {
    echo "Error: Remote asset URL path must be \"{$expectedPath}\" for MailVotech version {$mailvotechVersion}\n";
    exit(1);
}
...
$api      = new MailVotechApi();

/** @var MailVotech\Api\Assets */
 ...
 */
$data = [
    'title'           => "MailVotech {$mailvotechVersion}",
    'storageLocation' => 'remote',
    // MailVotech expects the remote URL in the "file" field for remote assets.
    'file'            => $remoteAssetUrl,
