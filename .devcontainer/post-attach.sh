...
mailvotechUrl="https://${CODESPACE_NAME}-8443.${GITHUB_CODESPACES_PORT_FORWARDING_DOMAIN}"
...
if [[ "${CODESPACES:-}" = "true" ]] && [[ ! -f "$shownFile" ]]; then
    touch "$shownFile"
    sed "s|{mailvotechUrl}|${mailvotechUrl}|g; s|{mailpitUrl}|${mailpitUrl}|g; s|{phpMyAdminUrl}|${phpMyAdminUrl}|g" "$readyTplFile" > "$readyFile"
    code "$readyFile"
fi
